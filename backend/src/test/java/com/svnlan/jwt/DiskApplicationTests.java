...
//        System.out.println(jwtService.getAccessToken(dto));
    ...
    public void unZipList(){
        ...
        String filePath = "D:\\钉钉文档\\FormatFactory_setup.rar";
        String fileName = "FormatFactory_setup.rar";
    ...
    private void compressFileCopyChangeList(List<CompressFileReader.FileNode> fileNodeList, String finalFolderPath, List<ChangeSourceVo> fileList, LoginUser loginUser, CheckFileDTO checkFileDTO
        ...
        for (CompressFileReader.FileNode node : fileNodeList) {
            if (node.isDirectory()) {
                ...
                try {//创建硬链
                    if (!linkFile.exists()) {
                        Files.copy(Paths.get(extractPath + node.getFileName()), Paths.get(finalFilePath),
            ...
            if (checkFileDTO.getFullName().equals(node.getFullName())){
                if (node.isDirectory()){
                    ...
                    try {//创建硬链
                        ...
                        LogUtil.error(e, "compressFileCopy 1 创建硬链接失败 finalFilePath=" + finalFilePath + ", orgPath=" + extractPath + node.getFileName());
                    } catch (Exception e){
                        LogUtil.error(e, "compressFileCopy 1 error  创建硬链接失败 finalFilePath"+ finalFilePath + ", orgPath=" + extractPath + node.getFileName());
                    }
                    return;
                }
