...
public class MsgWarningSchedule {
    ...
    public void msgWarningSchedule(){
        ...
        //更新前任务时间
        Integer j = commonScheduleDaoImpl.updateOperateTime(msgWarningSchedule);
        if (j < 1) {
            //LogUtil.error(msgWarningSchedule + "消息预警 schedule表更新失败");
            return;
        }
        // TODO 入参需修改
        PluginListVo pluginListVo = systemSettingService.getMsgWarningSetting(1L);
        ...
        // TODO 入参需修改
        Long body2 = logScheduleService.addLog(msgWarningSchedule, 1L);
        try {
