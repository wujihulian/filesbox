...
public class MsgWarningScheduleServiceImpl implements MsgWarningScheduleService {
    ...
    public String sendMsgWarming(PluginListVo pluginListVo){
        ...
        String timeStr = DateUtil.getYearMonthDayHMS(new Date(), "yyyy-MM-dd HH:mm");
        ...
        String systemName = systemOptionDaoImpl.getSystemConfigByKey("systemName");
        ...
        if (warnTypeList.contains("cpu")){
            ...
            Double useCup = getCpuUsedRate() * 100;
            if (useCup >= useRatio){
                // 达到预警条件
         ...
         */
           /* List<String> memoryList = ShellExecTool.callShellByExec("free -g");
            ...
            Double useMemoryRate = getMemoryUsedRate() * 100;
    ...
    public Double getDiskUsedRate(){
        return getDiskUsedRate(null);
    }
    @Override
    public Double getDiskUsedRate(JSONObject object){
        ...
        Double duRate0 = (totalSpaceUsed.doubleValue() / (diskDefaultSize * 1024 * 1024 * 1024) ) ;

        if (!ObjectUtils.isEmpty(object)){
            object.put("diskDefaultSize", diskDefaultSize * 1024 * 1024 * 1024);
            object.put("totalSpaceUsedSize", totalSpaceUsed);
            object.put("diskDefault", diskDefaultSize + "G");
            object.put("totalSpaceUsed", SystemInfoUtil.getFileSize(totalSpaceUsed));
        }
        Double dRate = Double.valueOf(df.format(duRate0));

        if (totalSpaceUsed > 0 && dRate <= 0 ){
            return 0.01;
        }
        return dRate ;
    }
    ...
    public Double getMemoryUsedRate(){
        ...
        try {
            ...
            if (!ObjectUtils.isEmpty(object) && !ObjectUtils.isEmpty(object.getDouble("memoryRate"))){
                rate = object.getDouble("memoryRate");
            }
        }catch (Exception e){
    ...
    public Double getCpuUsedRate() {
        ...
        Double rate = 0.0;
        try {
            object = SystemInfoUtil.getCpuInfo(object);
            if (!ObjectUtils.isEmpty(object) && !ObjectUtils.isEmpty(object.getDouble("cpuRate"))){
                rate = object.getDouble("cpuRate");
            }
        }catch (Exception e){
