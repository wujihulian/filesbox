...
public class NoticeUserServiceImpl implements NoticeUserService {
    ...
    public void buildNoticeAndUserRelation(Long id, LocalDateTime now, NoticeDTO dto, boolean isAll) {
        ...
        if (!isAll) {
            ...
            if (!CollectionUtils.isEmpty(deptIdList)) {
                // 根据部门id查询下面的用户
                List<Long> groupUserIds = groupDaoImpl.getUserIdByGroupId(deptIdList);
        ...
        Assert.isTrue(noticeUserDao.saveBatch(needToInsertList) == needToInsertList.size(), "保存通知用户关系失败");
    }
    ...
    public void buildRelationAfterLogin(Long userId, boolean needCheckUnique) {
        ...
        if (Objects.isNull(startQueryTime)) {
            ...
            Long createTimestamp = userDaoImpl.getCreateTime(userId);
            // 转化为 localDateTime
            startQueryTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(createTimestamp), ZoneId.of("+8"));
        }
        log.info("startQueryTime =>{}", startQueryTime);
        // 查询该用户的 roleId 和所在部门id
        Long roleID = Long.valueOf(userDaoImpl.getUserInfo(userId).getRoleID());
        List<Long> groupIdList = groupDaoImpl.getGroupIdByUserId(userId);
        ...
        if (needCheckUnique) {
            originNoticeIdList = context.select(NOTICE_USER.NOTICE_ID)
                    .from(NOTICE_USER)
                    .where(NOTICE_USER.USER_ID.eq(userId))
                    .fetch(NOTICE_USER.NOTICE_ID);
