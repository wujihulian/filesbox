...
@Slf4j
@Service
public class IoSourceServiceImpl implements IoSourceService {
    @Resource
    IoSourceMetaDao ioSourceMetaDaoImpl;
    @Resource
    private IoSourceDao ioSourceDao;
    @Resource
    private IoSourceDao ioSourceDaoImpl;

    @Resource
    private IoSourceEventDao ioSourceEventDao;

    private IoSourceMetaDao ioSourceMetaDao;
    ...
    public JSONObject fileInfoOverview() {
        // 文件数 目录数 视频数
        List<JSONObject> list = ioSourceDao.fileCount();
        // 当前剩余转码异常数
        Long exceptionCount = commonConvertDao.queryConvertExceptionCountInstant();
        // 计算14天前的时间戳和前一天的时间戳
        Integer daysBefore = 14;
        Pair<LocalDateTime, LocalDateTime> timeRange = calculateTimestampRange(daysBefore);
        // 文件操作数
        List<JSONObject> operateFileCountList = ioSourceEventDao.queryFileOperateCount(timeRange);
        // 视频操作数
        List<JSONObject> operateVideoFileCountList = ioSourceEventDao.queryVideoFileOperateCount(timeRange);
    ...
    public List<JSONObject> getFileTypeProportion() {
        return ioSourceDao.getFileTypeProportion();
    }
    ...
    public IOSourceVo getUserRootDirectory(Long userId) {
        ...
        if (Objects.isNull(rootSourceVo) || rootSourceVo.getStorageId() <= 0) {
            synchronized (IoSourceServiceImpl.class) {
                rootSourceVo = CaffeineUtil.USER_ROOT_DIRECTORY_CACHE.getIfPresent(userId);
                if (Objects.isNull(rootSourceVo) || rootSourceVo.getStorageId() <= 0) {
                    rootSourceVo = ioSourceDao.getUserRootDirectory(userId);
    ...
    public List<Long> getSourceIdByNameAndUserId(List<String> pathList, Long userId, List<Pair<IOSourceVo, List<Long>>> pairList, Long rootId, int isFolder) {
        ...
        List<IOSourceVo> sourceVo = ioSourceDao.getSourceByNameAndUserId(lastPathName, userId, isFolder);
     ...
     */
    private Pair<LocalDateTime, LocalDateTime> calculateTimestampRange(Integer daysBefore) {
    ...
    public List<JSONObject> getFileTypeProportionByUserId(Long userID) {
        return ioSourceDao.getFileTypeProportionByUserId(userID);
    }

    @Override
    public IOFile getFileContentByNameAndUserId(String fileName, Long userId, Boolean isVideoFile) {
        IOFile ioFile = ioSourceDao.getFileContentByNameAndUserId(fileName, userId, isVideoFile);
    ...
    public String deriveProperName(String parentLevel, String lastPath) {
        ...
        List<String> nameList = ioSourceDao.getDirectoryByParentLevelAndName(parentLevel, lastPath);
    ...
    public boolean createDirectory(Long userId, String name, String parentLevel, Long parentId) {
        ...
        Assert.isTrue(Objects.equals(ioSourceDao.insert(ioSourceInsert), 1), "保存 io_source 数据失败");
        ...
        ArrayList<IOSourceMeta> ioSourceMetas = Lists.newArrayList(
                new IOSourceMeta(ioSourceInsert.getId(), UserMetaEnum.namePinyin.getValue(), ChinesUtil.getPingYin(name)),
                new IOSourceMeta(ioSourceInsert.getId(), UserMetaEnum.namePinyinSimple.getValue(), ChinesUtil.getFirstSpell(name))
        );
        Assert.isTrue(Objects.equals(ioSourceMetaDaoImpl.batchInsert(ioSourceMetas), 2), "保存 io_source_meta 数据失败");
        // 写入 io_source_event 表
        IoSourceEvent ioSourceEventInsert = new IoSourceEvent(ioSourceInsert.getId(), parentId, userId, "create",
        ...
        Assert.isTrue(Objects.equals(ioSourceEventDao.insert(ioSourceEventInsert), 1), "保存 io_source_event 数据失败");
        log.info("sourceID => {} ioSourceEvent => {}", ioSourceInsert.getId(), ioSourceEventInsert.getId());
        return true;
    }
}
