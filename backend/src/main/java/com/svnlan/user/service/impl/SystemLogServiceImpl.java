...
//        PageHelper.startPage(systemLogDto.getCurrentPage(), systemLogDto.getPageSize());
        Long total = this.systemLogDao.getSystemLogListCount(map);

        map.put("startIndex", systemLogDto.getStartIndex());
        map.put("pageSize", systemLogDto.getPageSize());
        List<SystemLogVo> list = null ;
        if (0 < total) {
            list = this.systemLogDao.getSystemLogList(map);
        }
    ...
    private void systemLogReturnList(List<SystemLogVo> list){
        ...
        for (SystemLogVo vo : list){
            if (!ObjectUtils.isEmpty(vo.getCreateTime())){
                vo.setCreateTime(vo.getCreateTime() / 1000);
            }
        ...
        if (!CollectionUtils.isEmpty(pID)){
            ...
            pMap = CollectionUtils.isEmpty(moveList) ? null
                    : moveList.stream().collect(Collectors.toMap(IOSource::getId, Function.identity(), (v1, v2) -> v2));
        }
    ...
    private Pair<LocalDateTime, LocalDateTime> getTimeRange(int daysBefore) {
...
//        long startTimestamp = now.minusDays(daysBefore).withHour(0).withMinute(0).withSecond(0).withNano(0).toEpochSecond(ZoneOffset.of("+8"));
        LocalDateTime startTimestamp = now.minusDays(daysBefore).withHour(0).withMinute(0).withSecond(0).withNano(0);
        LocalDateTime endTimestamp = now.minusDays(1L).withHour(23).withMinute(59).withSecond(59);
//        long endTimestamp = now.minusDays(1L).withHour(23).withMinute(59).withSecond(59).toEpochSecond(ZoneOffset.of("+8"));
    ...
    private void resolvePlatformFromDesc(LocalDate startDate, List<List<Long>> lists, JSONObject jsonItem) {
        ...
        LocalDateTime date = jsonItem.getObject("createTime", LocalDateTime.class);
//        LocalDateTime currentLocalDateTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(date.getTime()),ZoneId.systemDefault());
        int days = Period.between(startDate, date.toLocalDate()).getDays();
    ...
    public void searchParamMap(Map<String, Object> map, SystemLogDto systemLogDto) {
        ...
        if (!ObjectUtils.isEmpty(systemLogDto.getTimeFrom())) {
            Date timeFrom = DateUtil.strToDate("yyyy-MM-dd", systemLogDto.getTimeFrom());
            map.put("minDate", timeFrom.getTime()/1000);
        }
        if (!ObjectUtils.isEmpty(systemLogDto.getTimeTo())) {
            Date timeTo = DateUtil.strToDate( "yyyy-MM-dd HH:mm:ss", systemLogDto.getTimeTo() + " 23:59:59");
    ...
    private List<SystemLogExportVO> systemLogReturnExportList(List<SystemLogVo> list){
        ...
        for (SystemLogVo vo : list){
            ...
            if (String.valueOf(vo.getCreateTime()).length() == 10){
                exportVO.setLoginTime(DateUtil.DateToStr(DateUtil.YYYY_MM_DD_HH_MM_SS, new Date(Long.parseLong(vo.getCreateTime()+"000"))));
            }else {
                exportVO.setLoginTime(DateUtil.DateToStr(DateUtil.YYYY_MM_DD_HH_MM_SS, new Date(vo.getCreateTime())));
            }
