...
/*
    ...
    public static Double jxMemoryRateValue(){
        DecimalFormat df = new DecimalFormat("#.00");
        List<String> memoryList = callShellByExec("free -g");
        Double freeMemoryRate = jxMemoryValue(memoryList, null);
        Double useMemoryRate0 = (1-freeMemoryRate) * 100;
        return Double.valueOf(df.format(useMemoryRate0));
    }
    public static Double jxCpuRateValue(){
        DecimalFormat df = new DecimalFormat("#.00");
        List<String> cpuList = callShellByExec("vmstat 1 1");
        String freeCpu = jxCpuValue(cpuList);
        Double useCup0 = 100 - Double.valueOf(freeCpu);
        return Double.valueOf(df.format(useCup0));
    }

    public static Double jxStorageRateValue(){
        /*DecimalFormat df = new DecimalFormat("#.00");
        List<String> memoryList = callShellByExec("dmidecode | grep -P -A5 \"Memory\\s+Device\" | grep Size | cut -f2 -d:");
        Long memory = jxStorageValue(memoryList);
    ...
    /*
 dmidecode | grep -P -A5 "Memory\s+Device" | grep Size | cut -f2 -d:
16384 MB
16384 MB
16384 MB
16384 MB
 */
    public static Long jxStorageValue(List<String> list){
        Long s = 0L;
        if (!CollectionUtils.isEmpty(list) && list.size() > 0){
            List<String> tList = Arrays.asList(list.get(0).split(" ")).stream().filter(n->!ObjectUtils.isEmpty(n)).map(String::valueOf).collect(Collectors.toList());
            if (!CollectionUtils.isEmpty(tList)){

                for (String st : tList){
                    if (!ObjectUtils.isEmpty(st) && StringUtil.isNumeric(st)){
                        s = Long.valueOf(st);
                        break;
                    }
                }
    ...
    public static String jxUptimeValue(List<String> list){
        Double time = 0d;
        if (!CollectionUtils.isEmpty(list) && list.size() > 0){
            List<String> tList = Arrays.asList(list.get(0).split(" ")).stream().filter(n->!ObjectUtils.isEmpty(n)).map(String::valueOf).collect(Collectors.toList());
            if (!CollectionUtils.isEmpty(tList)){
                for (String value : tList){
                    if (!ObjectUtils.isEmpty(value) && StringUtil.isNumeric(value)){
                        time = (Double.valueOf(value));
                        break;
                    }
                }
            }
        ...
        if (time > 0){
            return DateUtil.msecToTime(Long.valueOf(String.valueOf(time * 1000)));
        }else {
            return "-";
        }
    }
