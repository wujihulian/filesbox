            ...
            try {
                if (lock.tryLock()) {
                    asyncTaskExecutor.execute(() -> {
                        // 从数据库中查询出数据
                        List<VisitCountRecord> dbList = visitCountRecordDao.selectListByOsNameList(nullOsName,tenantId);
...
//                LocalDateTime datetime = (LocalDateTime) json.get("date");
                LocalDateTime datetime =  json.getObject("createTime", LocalDateTime.class);

                if (localDate.equals(datetime.toLocalDate())) {
                    isMatch = true;
