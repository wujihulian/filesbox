...
public class ShareServiceImpl implements ShareService {
    ...
    public void saveShare(ShareDTO shareDTO, LoginUser loginUser){
        ...
        fileOptionTool.checkSafeOperate(shareDTO, loginUser, shareDTO.getSourceID());
    ...
    public HomeExplorerResult getShareList(HomeExplorerDTO homeExplorerDTO, LoginUser loginUser){
        ...
        if (!ObjectUtils.isEmpty(homeExplorerDTO.getSortField()) && !ObjectUtils.isEmpty(homeExplorerDTO.getSortType())){
            ...
            hashMap.put("sortField", "io_source." + SourceFieldEnum.getSortField(homeExplorerDTO.getSortField()));
        }else {
            ...
            hashMap.put("sortField", "io_source." + SourceFieldEnum.getSortField(sortField));
                ...
                for (SystemSortVo vo : systemSortVo.getListSortList()){
                    if (vo.getSourceID().equals(String.valueOf(homeExplorerDTO.getSourceID()))){
                        ...
                        if (!ObjectUtils.isEmpty(vo.getListSortField())) {
                            hashMap.put("sortField", "io_source." + SourceFieldEnum.getSortField(vo.getListSortField()));
                        }
                        break;
                    }
                }
...
//        PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        Long total = this.shareDao.getShareListCount(hashMap);

        List<HomeExplorerVO> list = null;
        if (0 < total) {
            list = this.shareDao.getShareList(hashMap);
        }
...
//        PageInfo<HomeExplorerVO> pageInfo = new PageInfo<>(list);
    ...
    public ShareVo getShareFile(ShareDTO shareDTO, LoginUser loginUser){
        ...
        if (!ObjectUtils.isEmpty(homeExplorerVO.getCreateTime())){
            homeExplorerVO.setCreateTime(homeExplorerVO.getCreateTime() / 1000);
        }
    ...
    public HomeExplorerResult getLinkShareList(HomeExplorerDTO homeExplorerDTO){
        ...
        hashMap.put("pageSize", homeExplorerDTO.getPageSize());
//        PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        Long total = this.shareDao.getLinkShareListCount(hashMap);
        List<HomeExplorerVO> list = null;
        if (0 < total) {
            list = this.shareDao.getLinkShareList(hashMap);
        }
...
//        PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        Long total = this.shareDao.getShareToMeListCount(hashMap);
        List<HomeExplorerVO> list = null;
        if (0 < total) {

            hashMap.put("startIndex", homeExplorerDTO.getStartIndex());
            hashMap.put("pageSize", homeExplorerDTO.getPageSize());
            list = this.shareDao.getShareToMeList(hashMap);
        }
...
//        PageInfo<HomeExplorerVO> pageInfo = new PageInfo<>(list);
    ...
    public List<UserGroupVo> userGroupListBySearch(HomeExplorerDTO shareDTO, LoginUser loginUser){
        ...
        if (!CollectionUtils.isEmpty(userGroupList)){
            ...
            if (!"all".equals(authShowType)){
                ...
                if (!CollectionUtils.isEmpty(groupIDList)){
                    ...
                    List<UserGroupVo> list2 = groupDaoImpl.getGroupVoList(new ArrayList<>(groupIDList));
                ...
                // 获取所有部门
                list = groupDaoImpl.getMainGroupVoList(shareDTO);
            }
        }else if (isSystem){
            list = groupDaoImpl.getMainGroupVoList(shareDTO);
        }

        Group g = groupDaoImpl.getTopGroup();
        ...
        if (!CollectionUtils.isEmpty(list)){
            ...
            if (!CollectionUtils.isEmpty(gidList)){
                List<Group> countList = groupDaoImpl.getGroupUserCountList(gidList);
    ...
    public SelectAuthVo userGroupListByGroup(HomeExplorerDTO shareDTO, LoginUser loginUser){
        ...
        Group g = groupDaoImpl.getGroupInfoByID(shareDTO.getGroupID());
        ...
        list = groupDaoImpl.getGroupList(paramDto);
