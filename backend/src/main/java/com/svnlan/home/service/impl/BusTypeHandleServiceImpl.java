...
import com.svnlan.home.utils.AsyncDoForImageUtil;
...
public class BusTypeHandleServiceImpl implements BusTypeHandleService {
    ...
    @Resource
    AsyncDoForImageUtil asyncDoForImageUtil;
...
将PDF、PPT、WORD、EXCEL原文件转成图片  homework
            ...
            if (Arrays.asList(GlobalConfig.IMAGE_TYPE_ARR).contains(sourceSuffix)){
                /* boolean async = false;
                if (!isBefore && !ObjectUtils.isEmpty(uploadDTO.getAsync()) && uploadDTO.getAsync()
                        && Arrays.asList(GlobalConfig.IMAGE_TYPE_PHOTOGRAPH).contains(commonSource.getFileType())){
                    async = true;
                }
                if (async) {
                    // 上传异步处理缩略图
                    asyncDoForImageUtil.asyncDoForImage(uploadDTO,  commonSource);
                }else {*/
