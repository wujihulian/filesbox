...
public class VideoGetServiceImpl implements VideoGetService {
    ...
    public List getVideoShearListAll(VideoCommonDto videoCommonDto){
        ...
        if (first){
            ...
            for (int i = 0 ; i < num; i++){
                ...
                if (!isExist) {
                    LogUtil.info("cutList tempImgPath=" + tempImgPath + "， beginTime=" + DateUtil.getYearMonthDayHMS(new Date(), DateUtil.YYYY_MM_DD_HH_MM_SS) );
                    ...
                    LogUtil.info("cutList tempImgPath=" + tempImgPath + "， endTime=" + DateUtil.getYearMonthDayHMS(new Date(), DateUtil.YYYY_MM_DD_HH_MM_SS) );
                    futures.add(future);
                }
                    ...
                    for (Future<?> future : futures) {
                        while (true) {//CPU高速轮询：每个future都并发轮循，判断完成状态然后获取结果，这一行，是本实现方案的精髓所在。即有10个future在高速轮询，完成一个future的获取结果，就关闭一个轮询
                            if (future.isDone() && !future.isCancelled()) {//获取future成功完成状态，如果想要限制每个任务的超时时间，取消本行的状态判断+future.get(1000*1, TimeUnit.MILLISECONDS)+catch超时异常使用即可。
                                ...
                                LogUtil.info("getVideoShearListAll 任务i=" + future.get() + "获取完成!" + DateUtil.getYearMonthDayHMS(new Date(), DateUtil.YYYY_MM_DD_HH_MM_SS));
        ...
        asyncCutImgUtil.AsyncExecVideoShearList(videoCommonDto, defaultFirstPath);
    ...
    public String getVideoShearImg(HttpServletResponse response, VideoCommonDto videoCommonDto, String passedFileName, Map<String, Object> resultMap){
        ...
        if (ObjectUtils.isEmpty(passedFileName)){
            throw new SvnlanRuntimeException(CodeMessageEnum.shareErrorParam.getCode());
        }
        if (ObjectUtils.isEmpty(videoCommonDto.getKey()) && ObjectUtils.isEmpty(videoCommonDto.getSid())){
            throw new SvnlanRuntimeException(CodeMessageEnum.shareErrorParam.getCode());
        }

        String videoPathOrg = "";
        if (!ObjectUtils.isEmpty(videoCommonDto.getKey())){
            ...
            CommonSource source = ioSourceDao.getSourceInfo(videoCommonDto.getSid());
            if (ObjectUtils.isEmpty(source)){
                return "";
            }
            videoPathOrg = source.getPath();
        }
        String firstPath = FileUtil.getFirstStorageDevicePath(videoPathOrg);
        File file = null;
        if (!ObjectUtils.isEmpty(videoCommonDto.getNameSuffix()) && videoPathOrg.indexOf(firstPath + "/private/cloud") >= 0){
            ...
            if (!file.exists()){
                ...
                // 异步生成缩略图
                asyncCutImgUtil.AsyncExecImageThumb(videoPathOrg, stringRedisTemplate);
...
//                return null;
        ...
        if (videoCommonDto.getCutList().size() > 1){
            ...
            if (!CollectionUtils.isEmpty(cutList) && !ObjectUtils.isEmpty(tempPathFinish)) {
                ...
                ,fileOptionTool, loginUser.getUserID(), loginUser.getTenantId());
            }
    ...
    public Map<String, Object> splitVideoSave(VideoCommonDto checkFileDTO, LoginUser loginUser){
        ...
        List<VideoCutDto> cutList = ObjUtil.objectToList(cutMap.get("cutDtoList"), VideoCutDto.class);
         List<CommonSource> convertList = new ArrayList<>();
        LogUtil.info("splitVideoSave addCommonSource beginTime=" + DateUtil.getYearMonthDayHMS(new Date(), DateUtil.YYYY_MM_DD_HH_MM_SS));
        ...
        LogUtil.info("splitVideoSave addCommonSource endTime=" + DateUtil.getYearMonthDayHMS(new Date(), DateUtil.YYYY_MM_DD_HH_MM_SS));
        cutMap.put("convertList",convertList);
        return cutMap;
    }
...
//                return null;
        ...
        // 转码
        asyncUtil.cutVideoMerge(cutList, finalFilePath, fileType, "", convertList, "mergeVideo", null, 0L, tenantId);
...
//                return null;
    ...
    @Override
    public String fileOutPath(VideoCommonDto videoCommonDto, String passedFileName){
        if (ObjectUtils.isEmpty(videoCommonDto.getSourceID())){
            throw new SvnlanRuntimeException(CodeMessageEnum.paramFormatError.getCode());
        }
        LoginUser loginUser = null;
        if (!ObjectUtils.isEmpty(videoCommonDto.getKey())) {
            loginUser = loginUserUtil.getLoginUserByToken(HttpUtil.getRequest(), videoCommonDto.getKey());
        }
        ...
        CommonSource source = fileOptionTool.getSourceInfo(videoCommonDto.getSourceID());
        if (ObjectUtils.isEmpty(source) || ObjectUtils.isEmpty(source.getIsExistFile()) || source.getIsExistFile().intValue() == 0){
            return "";
        }
        String ext = passedFileName.substring(passedFileName.lastIndexOf(".") + 1);
        String fileType = source.getPath().substring(source.getPath().lastIndexOf(".") + 1);
