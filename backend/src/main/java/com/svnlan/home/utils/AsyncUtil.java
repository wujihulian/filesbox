...
public class AsyncUtil {
    ...
    public void asyncZipFile(CheckFileDTO checkFileDTO, List<IOSourceVo> copyListByLevel) {
        ...
        List<IOSourceVo> zipAttachments = new ArrayList<>();
        ...
        for (IOSourceVo vo : attachments){
            ...
            if (0 == vo.getIsFolder()){
                try {
                    checkFile = new File(vo.getPath());
                    if (!checkFile.exists()){
                        LogUtil.error("压缩排除文件损坏 文件不存在 vo=" + JsonUtils.beanToJson(vo));
                        continue;
                    }
                }catch (Exception e){
                    LogUtil.error(e, "压缩排除文件损坏 vo=" + JsonUtils.beanToJson(vo));
                    continue;
                }
            }
            if (1 == vo.getIsFolder()){
            ...
            zipAttachments.add(vo);
        }
        if (!CollectionUtils.isEmpty(zipAttachments)) {
            // 按父类升序
            zipAttachments = zipAttachments.stream().sorted(Comparator.comparing(IOSourceVo::getParentLevel)).collect(Collectors.toList());
        }
        LogUtil.info("asyncZipFile  zipAttachments=" + JsonUtils.beanToJson(zipAttachments));

        /* 优化zip 按目录结构创建并复制文件
        ZipUtils.addFile(attachments, finalFolderPath + tempFolder + "/");*/
        // 按目录结构创建
        ZipUtils.addFileNew(zipAttachments, finalFolderPath + tempFolder + "/");

        long totalLength = 0;
        for (IOSourceVo vo : zipAttachments){
            if (0 == vo.getIsFolder()){
                totalLength = totalLength + (!ObjectUtils.isEmpty(vo.getFileSize()) && vo.getFileSize() > 0 ? vo.getFileSize() : vo.getSize());
            }
        }
        LogUtil.info("zipFile totalLength="+totalLength+"  finalFilePath=" + finalFilePath + ",zipAttachments=" + JsonUtils.beanToJson(zipAttachments));
        /**  打包 */
        try {
            ...
            ZipUtils.toZipNewList(finalFolderPath + tempFolder + "/", finalFilePath, stringRedisTemplate
                    , checkFileDTO, true, zipAttachments, mainAttsMap, totalLength);
        ...
        if (size > 0 && !isDown){
            ...
            sourceUpdate.setId(checkFileDTO.getBusId());
        ...
        stringRedisTemplate.opsForValue().set(GlobalConfig.progress_key_zip_file + checkFileDTO.getTaskID(), String.valueOf(100), 5, TimeUnit.HOURS);
    ...
    public void genInformationPage(CommonInfoDto infoDto, LoginUser loginUser) {
        ...
        this.saveForSEO(infoList, loginUser.getTenantId(),infoDto.getServiceName());
        try {
    ...
    public void saveForSEO(List<CommonSeo> seoList, Long tenantId, String serverName) {
        ...
        String seoKeyword = "";
        if (isInformation){
            seoKeyword = systemOptionDaoImpl.getSystemConfigByKey("seo", tenantId);
        }
        //域名循环
        ...
        for (String clientType : GlobalConfig.DESIGN_WEB_CLIENT_TYPE_ARR) {
            //String domainAndClientPath = serverName + "/" + clientType;
            //循环
            for (CommonSeo seo : seoList) {
                ...
                if (!isUrlInfo ) { //并且非动态的
                    if (!StringUtil.isEmpty(seo.getDetail())){
                        html = html.replace("<!-- /*article.content*/ -->", this.handleDetail(seo, serverName));
                    }
    ...
    private String handleDetail(CommonSeo seo, String serverName) {
        String detail = "";
        ...
        if (seo.getTypeStr().equals("information")) {
            ...
            if (Boolean.TRUE.equals(seo.getSingle())){
                List<String> serverNameList = asyncDesignUtil.getSaveDesignServerNameList(serverName);
                File file = null;
                for (String serverNamePath : serverNameList){
                    String detailFilePath = designBasePath  + serverNamePath + "/pc/" + seo.getTypeStr() + "/__detail_" + seo.getId() + "__.txt";
                    LogUtil.info("handleDetail detailFilePath=" + detailFilePath);
                    file = new File(detailFilePath);
                    if (!file.getParentFile().exists()){
                        file.getParentFile().mkdirs();
                    }
                ...
                String serverNamePath = asyncDesignUtil.getInfoHtmlPathByConfig(serverName);
                String detailFilePath = designBasePath  + serverNamePath + "/pc/" + seo.getTypeStr() + "/__detail_" + seo.getId() + "__.txt";
                LogUtil.info("handleDetail detailFilePath=" + detailFilePath);
    ...
    public void cutVideoMerge(List<String> cutList, String finishPath, String suffix, String tempPath, List<CommonSource> convertList, String otherType
            , FileOptionTool fileOptionTool, long userID, Long tenantId) {
