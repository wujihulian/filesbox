...
public class FileUtil {
    ...
    public static String getFileExtension(String fileName) {
        ...
        if (fileName.indexOf(".") == 0){
            ...
            return getFileExtension(fileName1);
        }
        ...
        switch (busTypeEnum) {
            ...
            case CLOUD:
                ...
                if (StringUtil.isEmpty(path) && Arrays.asList(GlobalConfig.IMAGE_TYPE_ARR).contains(suffix)) {
                    path = (showPath + sourcePath).replace(firstPath + "/private/", firstPath + "/common/");
                }
                break;
            default:
                path = null;
    ...
    public static String getFirstStorageDevicePath(String path){
        ...
        List<String> pathList =  Arrays.asList(path.split("/")).stream().filter(n->!ObjectUtils.isEmpty(n)).collect(Collectors.toList());
        if (!CollectionUtils.isEmpty(pathList) && pathList.size() > 2){
            if (StringUtil.isNumeric(pathList.get(1))){
                return "/" + pathList.get(0) + "/" + pathList.get(1);
            }else {
    ...
    public static Boolean generateFile(String prefix, InputStream is, String fileName) {
        ...
        try {
            ...
            LogUtil.error(e, prefix + "将InputStream写入文件发生异常 fileName=" + fileName);
        } finally {
            if (null != in) {
                try {
                    in.close();
