...
public class AsyncCountSizeUtil {
    ...
    public void asyncCountSize(CheckFileDTO updateFileDTO) {
        ...
        for (IOSource source : list){
            ...
            countList =  operationsDao.getSourceListByLevelToContSize(source.getParentLevel()+source.getId()+",", ObjectUtils.isEmpty(updateFileDTO.getStatus()) ? 0 : updateFileDTO.getStatus());
            try {
