...
public class AsyncSourceFileOtherUtil {
    ...
    public void asyncAddFileMeta(CommonSource commonSource, IOFile ioFile) {
        ...
        if (Arrays.asList(GlobalConfig.AUDIO_VIDEO_SHOW_TYPE_ARR).contains(commonSource.getFileType().toLowerCase())
            ...
            // 其他的异步执行
        }else if("oexe".equals(commonSource.getFileType().toLowerCase())){
            fileMetaVo.setOexeContent(ObjectUtils.isEmpty(commonSource.getOexeContent()) ? null : commonSource.getOexeContent());
        }
        fileMeta.setValue(!ObjectUtils.isEmpty(fileMetaVo) ? JsonUtils.beanToJson(fileMetaVo) : "");
        fileMeta.setTenantId(commonSource.getTenantId());
        try {
    ...
    public void asyncAddSourceMeta(CommonSource source) {
        ...
        pyList.add(new IOSourceMeta(source.getSourceID(), MetaEnum.namePinyin.getValue(), ChinesUtil.getPingYin(source.getName()), source.getTenantId()));
        pyList.add(new IOSourceMeta(source.getSourceID(), MetaEnum.namePinyinSimple.getValue(), ChinesUtil.getFirstSpell(source.getName()), source.getTenantId()));

        if (!ObjectUtils.isEmpty(source.getDingEventJson())){
            pyList.add(new IOSourceMeta(source.getSourceID(), "dingEventDetail", source.getDingEventJson(), source.getTenantId()));
        }
        if (!CollectionUtils.isEmpty(pyList)) {
            ...
            try {
                ...
                LogUtil.error(e, " setUserDefaultSource setSourcePinYin meta 1 error pyList=" + JsonUtils.beanToJson(pyList));
            }
        }
    }
    ...
    @Async(value = "asyncTaskExecutor")
    public void asyncUpdateOexeFileMeta(long fid, String value, String oexeContent) {
        if (ObjectUtils.isEmpty(oexeContent)){
            LogUtil.info("异步更新oexe文件 内容为空");
            return;
        }
        FileMetaVo fileMetaVo = JsonUtils.jsonToBean(value, FileMetaVo.class);
        if (ObjectUtils.isEmpty(fileMetaVo)){
            fileMetaVo = new FileMetaVo();
        }
        fileMetaVo.setOexeContent(oexeContent);
        ioFileDao.updateOneFileUrlValue(fid, JsonUtils.beanToJson(fileMetaVo));
    }
}
