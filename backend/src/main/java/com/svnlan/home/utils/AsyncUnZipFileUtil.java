...
public class AsyncUnZipFileUtil {
    ...
    , UploadDTO uploadDTO, CommonSource parentSource, List<String> sourceNameList, Integer targetType, String serverUrl,Long tenantId) {
        commonSource.setTenantId(tenantId);
        /** 解压整个文件 */
        ...
        if (!CollectionUtils.isEmpty(convertList)) {
            for (CommonSource source : convertList) {
                ...
                source.setUpdateParentSize(true);
