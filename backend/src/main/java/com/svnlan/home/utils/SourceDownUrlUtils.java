...
public class SourceDownUrlUtils {
    ...
    public int deleteCommonDownload() {
        boolean check = stringRedisTemplate.opsForValue().setIfAbsent("lookDeleteCommonDownload", "1", 1, TimeUnit.MINUTES);
        if (!check){
            LogUtil.info("deleteCommonDownload 删除普通下载, 一分钟锁定");
            return 0;
        }
        ...
        if (CollectionUtils.isEmpty(members)){
            LogUtil.info("deleteCommonDownload 删除普通下载, 集合空");
            return 0;
        }
