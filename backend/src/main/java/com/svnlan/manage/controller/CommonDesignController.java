...
public class CommonDesignController {
    ...
    @RequestMapping(value = "/api/disk/addDesignCopy", method = RequestMethod.POST)
    public String addDesignCopy(@Valid @RequestBody AddDesignCopyDTO addDesignCopyDTO){
        Result result;
        try {
            Long designId = commonDesignService.addDesignCopy(addDesignCopyDTO);
            result = new Result(true, CodeMessageEnum.success.getCode(), designId);
        } catch (SvnlanRuntimeException e){
            result = new Result(false, e.getMessage(), e.getErrorCode(), null);
        } catch (Exception e){
            LogUtil.error(e, "addDesignCopy 网校复制一个装扮失败，dto: " + JsonUtils.beanToJson(addDesignCopyDTO));
            result = new Result(false, CodeMessageEnum.system_error.getCode(), null);
        }
        return JsonUtils.beanToJson(result);
    }
