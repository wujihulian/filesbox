...
public class CommonInfoController {
    ...
    /**
     * @Description: 首页获取资讯列表, 不需要验证
     * @Return: java.lang.String
     * @Modified:
     */
    @RequestMapping(value = "/api/disk/firstHomepageDetail", method = RequestMethod.GET)
    public String firstHomepageDetail(FirstpageInfoIdVO firstpageInfoIdVO, HttpServletRequest request) {
        List<CommonInfoVo> list;
        Result result;
        String prefix = this.jwtTool.findApiPrefix(request);
        String paramTip = "";
        try {
            paramTip = String.format(" >>> firstpageInfoIdVO:%s", JsonUtils.beanToJson(firstpageInfoIdVO));
            // 查询某一条咨询信息
            list = commonInfoService.firstHomepageDetail(firstpageInfoIdVO);
            result = Result.returnSuccess(list);
        } catch (SvnlanRuntimeException e) {
            LogUtil.error(e, prefix + "SvnlanRuntimeException" + paramTip);
            result = Result.returnSvnException(e);
