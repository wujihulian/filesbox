...
public class CommonInfoServiceImpl implements CommonInfoService, VisitHandler {
    ...
    @Resource
    AsyncDesignUtil asyncDesignUtil;
    @Resource
    ...
    public PageResult getInfoList(CommonInfoDto infoDto, LoginUser loginUser) {
        ...
        Long total = commonInfoDao.getInfoVoListByParamTotal(paramMap);

        paramMap.put("startIndex", infoDto.getStartIndex());
        paramMap.put("pageSize", infoDto.getPageSize());
//        PageHelper.startPage(infoDto.getCurrentPage(), infoDto.getPageSize());
        List<CommonInfoVo> list = null;
        if (0 < total) {
            list = commonInfoDao.getInfoVoListByParam(paramMap);
        }
...
//        pageResult.setTotal(pageInfo.getTotal());
    ...
    public Long saveCommonInfo(CommonInfoDto infoDto, LoginUser loginUser) {
        ...
        if (!ObjectUtils.isEmpty(commonInfo.getInfoID())) {
            ...
            if (1 == commonInfo.getStatus().intValue()) {
                // 生成静态页面
                infoDto.setServiceName(loginUserUtil.getServerName());
    ...
    public void delInfoFile(Long infoID) {
        ...
        List<String> serverNameList = asyncDesignUtil.getSaveDesignServerNameList(loginUserUtil.getServerName());
        for (String serverName : serverNameList) {
            try {
                String detailFilePath = firstPath + "/design/home/" + serverName + "/pc/information/__detail_" + infoID + "__.txt";
    ...
    private void associationTag(CommonInfoDto infoDto, Long infoID, boolean isDel) {
        ...
        if (!ObjectUtils.isEmpty(infoDto.getTagIds())) {
            ...
            if (!CollectionUtils.isEmpty(tagIds)) {
                List<UserFavModel> list = new ArrayList<>();
    ...
    public CommonInfoVo getCommonInfo(CommonInfoDto infoDto, LoginUser loginUser) {
        ...
        String detail = DesignUtil.getDetail(info, getInfoHtmlPathByConfig());
    ...
    public String getInfoHtmlPathByConfig(){
        return asyncDesignUtil.getInfoHtmlPathByConfig(loginUserUtil.getServerName());
    }
...
//        commonHomepageInfoDao.insertViewCount(homepageInfoId);
        ...
        String detail = DesignUtil.getDetail(homepageDetail, getInfoHtmlPathByConfig());
        ...
        // 查询当前登录人是否已经点赞了该资讯
        Condition condition = DSL.trueCondition().and(USER_COMMON_INFO.INFO_ID.eq(homepageDetail.getInfoID()));
        if (Objects.isNull(loginUser)) {
            // 匿名用户
            condition = condition.and(USER_COMMON_INFO.IP.eq(getRequestIp()));
        } else {
            condition = condition.and(USER_COMMON_INFO.USER_ID.eq(loginUser.getUserID()));
        }
        UserCommonInfo userCommonInfo = context.select()
                .from(USER_COMMON_INFO)
                .where(condition)
                .fetchOneInto(UserCommonInfo.class);
...
//                    .last(" limit 1"));
            ...
            originUserCommonInfo = context.select()
                    .from(USER_COMMON_INFO)
                    .where(USER_COMMON_INFO.INFO_ID.eq(id))
                    .and(USER_COMMON_INFO.IP.eq(ip))
                    .limit(1)
                    .fetchOneInto(UserCommonInfo.class);
...
//                    .last(" limit 1"));
            ...
            originUserCommonInfo = context.select()
                    .from(USER_COMMON_INFO)
                    .where(USER_COMMON_INFO.INFO_ID.eq(id))
                    .and(USER_COMMON_INFO.USER_ID.eq(userId))
                    .limit(1)
                    .fetchOneInto(UserCommonInfo.class);
        }
    ...
    public void operateLike(Long infoId, Integer isLike) {
        ...
        if (Objects.equals(userId, 0L)) {
            ...
            String ip = getRequestIp();
            // 先查询是否存在记录
            doOperateLike(infoId, "ip", ip, isLike);
        } else {
            // 表示是登录用户
            doOperateLike(infoId, "userId", userId, isLike);
        }
    }

    @Transactional(rollbackFor = Exception.class)
    public void doOperateLike(Long infoId, String columnName, Object columnValue, Integer isLike) {
...
//                .eq(column, columnValue));
        UserCommonInfo originCommonInfo = context.select()
                .from(USER_COMMON_INFO)
                .where(USER_COMMON_INFO.INFO_ID.eq(infoId))
                .and(Objects.equals("ip", columnName) ? USER_COMMON_INFO.IP.eq((String) columnValue) : USER_COMMON_INFO.USER_ID.eq((Long) columnValue))
                .fetchOneInto(UserCommonInfo.class);
    ...
    /**
     * @Description: 首页获取资讯列表不需要验证
     * @Modified:
     */
    @Override
    public List<CommonInfoVo> firstHomepageDetail(FirstpageInfoIdVO firstpageInfoIdVO){
        if (firstpageInfoIdVO.getList() != null) {
            List<CommonInfoVo> commonHomepageInfos = commonInfoDao.selectByInfoIds(firstpageInfoIdVO.getList());
            return getCommonHomepageInfos(commonHomepageInfos, null);
        } else {
            Long tenantId = tenantUtil.getTenantIdByServerName();
            int i = 0;
            if (firstpageInfoIdVO.getEndNum() != null && firstpageInfoIdVO.getBeginNum() != null) {
                //获取区间条数
                i = firstpageInfoIdVO.getEndNum() - firstpageInfoIdVO.getBeginNum() + 1;
            }
            if (ObjectUtils.isEmpty(firstpageInfoIdVO.getBeginNum())) {
                firstpageInfoIdVO.setBeginNum(0);
            }
            if (ObjectUtils.isEmpty(firstpageInfoIdVO.getEndNum())) {
                i = 4;
            }
            JSONObject map = new JSONObject();
            map.put("i", i);
            map.put("tenantId", tenantId);
            map.put("beginNum", (firstpageInfoIdVO.getBeginNum() - 1) < 0 ? 0 : firstpageInfoIdVO.getBeginNum() - 1);
            if (!ObjectUtils.isEmpty(firstpageInfoIdVO.getInfoType())){
                map.put("infoType", firstpageInfoIdVO.getInfoType());
            }
            List<CommonInfoVo> commonHomepageInfos;
            if (firstpageInfoIdVO.getInfoTypeId() != null && "1".equals(firstpageInfoIdVO.getIsAll())) {
                map.put("infoTypeId", firstpageInfoIdVO.getInfoTypeId());
                //根据类别获取
                CommonInfoType commonHomepageInfoType = commonInfoTypeDao.selectById(firstpageInfoIdVO.getInfoTypeId());
                if (!ObjectUtils.isEmpty(commonHomepageInfoType)) {
                    map.put("sequence", commonHomepageInfoType.getParentLevel());
                }
                //
                commonHomepageInfos = commonInfoDao.selectByParam(map);
                LogUtil.info("selectByParam 1 param=" + JsonUtils.beanToJson(map) + "，commonHomepageInfos=" + JsonUtils.beanToJson(commonHomepageInfos));
            } else if (firstpageInfoIdVO.getInfoTypeId() != null && "0".equals(firstpageInfoIdVO.getIsAll())) {
                map.put("infoTypeId", firstpageInfoIdVO.getInfoTypeId());
                //
                commonHomepageInfos = commonInfoDao.selectByParam(map);
                LogUtil.info("selectByParam 2 param=" + JsonUtils.beanToJson(map) + "，commonHomepageInfos=" + JsonUtils.beanToJson(commonHomepageInfos));
            } else {
                //如果分类为空，则按照时间倒叙显示10条
                commonHomepageInfos = commonInfoDao.selectByParam(map);
                LogUtil.info("selectByParam 3 param=" + JsonUtils.beanToJson(map) + "，commonHomepageInfos=" + JsonUtils.beanToJson(commonHomepageInfos));
            }
            return getCommonHomepageInfos(commonHomepageInfos, firstpageInfoIdVO);
        }
    }

    private List<CommonInfoVo> getCommonHomepageInfos (List<CommonInfoVo> commonHomepageInfos
                    , FirstpageInfoIdVO firstpageInfoIdVO) {
        Map<String, Object> map;
        for (CommonInfoVo commonHomepageInfo : commonHomepageInfos) {
            if (!ObjectUtils.isEmpty(commonHomepageInfo.getThumb())) {
                commonHomepageInfo.setThumb(FileUtil.getShowImageUrl(commonHomepageInfo.getThumb(), "info" + commonHomepageInfo.getInfoID() + ".png"));
            }
            if (!ObjectUtils.isEmpty(commonHomepageInfo.getAvatar())) {
                commonHomepageInfo.setAvatar(FileUtil.getShowAvatarUrl(commonHomepageInfo.getAvatar(), commonHomepageInfo.getUserName()));
            }
            if ("".equals(commonHomepageInfo.getComputerPicPath())) {
                if (!ObjectUtils.isEmpty(firstpageInfoIdVO) && !ObjectUtils.isEmpty(firstpageInfoIdVO.getType()) && firstpageInfoIdVO.getType() == 1) {
                    commonHomepageInfo.setComputerPicPath(null);
                } else {
                    List<Map<String, Object>> list = new ArrayList<>();
                    map = new HashMap<>();
                    map.put("commonSourceId", 0);
                    map.put("sourcePath", "");
                    list.add(map);
                    commonHomepageInfo.setComputerPicPath(JsonUtils.beanToJson(list));
                }
            }
        }
        //处理资讯集合的浏览数（优先拿缓存值置）
        //this.commonTool.handledHomePagesForViewCount(commonHomepageInfos);
        return commonHomepageInfos;
    }
}
