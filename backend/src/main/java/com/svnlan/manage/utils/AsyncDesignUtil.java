...
public class AsyncDesignUtil {
    ...
    public Boolean  saveDesignFile(Long designId, Map<String, Object> result, String serverName) {
        return saveDesignFile( designId, result, serverName, null);
    }
    public Boolean saveDesignFile(Long designId, Map<String, Object> result, String serverName, List<String> serverNameList) {
        //获取当前装扮
        ...
        // 保持地址规则：单租户模式默认infoHtmlPath配置值 多租户则查询是否设置外网、内网，没有设置则用域名
        // tenantTypeConfig  1 单租户模式 2 平台模式
        serverNameList = CollectionUtils.isEmpty(serverNameList) ? this.getSaveDesignServerNameList(serverName) : serverNameList;
        for (String network : serverNameList){

            //保存目录
            String savePath =  designBasePath +  network + "/" + clientTypeDirectory;
    ...
    public List<String> getSaveDesignServerNameList(String serverName){
        // 保持地址规则：单租户模式默认infoHtmlPath配置值 多租户则查询是否设置外网、内网，没有设置则用域名
        // tenantTypeConfig  1 单租户模式 2 平台模式
        String tenantType = "1";
        String networkConfig = null;
        List<OptionVo> list = systemOptionDao.getSystemConfigByKeyListBy(Arrays.asList("tenantTypeConfig","networkConfig"));
        for (OptionVo vo : list) {
            if ("tenantTypeConfig".equals(vo.getKeyString()) && !ObjectUtils.isEmpty(vo.getValueText())) {
                tenantType = vo.getValueText();
            }
            if ("networkConfig".equals(vo.getKeyString()) && !ObjectUtils.isEmpty(vo.getValueText())) {
                networkConfig = vo.getValueText();
            }
        }
        List<String> serverNameList = new ArrayList<>();
        if ("2".equals(tenantType)){
            if (!ObjectUtils.isEmpty(networkConfig) && !"{}".equals(networkConfig)) {
                try {
                    JSONObject obj = JSONObject.parseObject(networkConfig);
                    serverNameList.add(obj.getString("external"));
                    serverNameList.add(obj.getString("intranet"));
                }catch (Exception e){
                }
            }
            if (CollectionUtils.isEmpty(serverNameList) || serverNameList.size() <= 0 || !serverNameList.contains(serverName)){
                serverNameList.add(serverName);
            }
        }else {
            serverNameList.add(infoHtmlPath);
        }
    ...
    public String getInfoHtmlPathByConfig(String serverName){
        String tenantType = systemOptionDao.getSystemConfigByKey("tenantTypeConfig");
        return  "1".equals(tenantType) ? infoHtmlPath : serverName;
    }
