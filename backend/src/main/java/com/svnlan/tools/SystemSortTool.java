...
public class SystemSortTool {
    ...
    public SystemSortVo getUserSort(Long userID){
        ...
        List<UserOptionVo> sortOptionList =  userOptionDaoImpl.getUserSortConfig(userID, UserOptionEnum.sortKeyList);
    ...
    public List<HomeExplorerVO> getUserGroupSourceList(Long userID){
        return homeExplorerDaoImpl.getUserGroupSourceList(userID);
    }

    /** 系统管理员可以看到所有 */
    public List<HomeExplorerVO> getSystemGroupSourceList(Long tenantId){
        return homeExplorerDaoImpl.getSystemGroupSourceList(tenantId);
    }
    ...
    public String getCdnPath(String hardLinkPath, String fileName){
        ...
        if (ObjectUtils.isEmpty(cdnDomain)){
            cdnPath = HttpUtil.getReqRootUrl(null, getNetworkConfig()) + FileUtil.getShowImageUrlDown(hardLinkPath, fileName) ;
        }else {
    ...
    public List<String> getNetworkConfig(){
    ...
    /** fileRepeat 1 覆盖 2 重命名 3 跳过*/
    public String getUserUploadFileRepeatTypeRedis(long userId){

        HashOperations<String, String, String> operations = stringRedisTemplate.opsForHash();
        String key = GlobalConfig.user_upload_file_repeat_type_key;
        String type = operations.get(key, String.valueOf(userId));
        if (!ObjectUtils.isEmpty(type)){
            return type;
        }
        type = getUserUploadFileRepeatType(userId);
        operations.put(key, String.valueOf(userId), type);

        Long hashLength = operations.size(key);
        if (hashLength <= 0){
    ...
    /** fileRepeat 1 覆盖 2 重命名 3 跳过*/
    public String getUserUploadFileRepeatType(long userId){
        OptionVo fileRepeat = userOptionDaoImpl.getUserConfigVoByKey(userId, "fileRepeat");
        if (!ObjectUtils.isEmpty(fileRepeat) && !ObjectUtils.isEmpty(fileRepeat.getValueText())){
            return fileRepeat.getValueText();
        }
        return "2";
    }
}
