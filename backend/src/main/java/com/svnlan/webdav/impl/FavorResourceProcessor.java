...
// s.sourceID, s.type, s.isFolder, s.name, s.parentLevel, s.fileID, i.path
        ...
        if (sourceIdList.size() <= perMaxQuery) {
            List<IOSource> ioSources = ioSourceDao.selectSimpleSourceById(sourceIdList);
...
//                stringLongMap.put(ioSource.getName(), ioSource);
            ...
            if (!CollectionUtils.isEmpty(fileIdList)) {
                ...
                List<IOFile> ioFileList = ioFileDao.selectPathList(fileIdList);
                for (IOFile ioFile : ioFileList) {
                    filePathMap.put(ioFile.getId(), ioFile.getPath());
                }
            }
            ...
            for (IOSource ioSource : ioSources) {
                ...
                JSONObject jsonObject = new JSONObject().fluentPut("sourceID", ioSource.getId())
