...
//        log.info("111getFile path => {} ", path);
    ...
    public void cancelUpload() throws LockedException, ServerException {
        ...
        if (Objects.nonNull(ioFile) && Objects.nonNull(ioFile.getId())) {
            wrapper.ioFileDao.deleteById(ioFile.getId());
        }
    }
    ...
    public boolean beforeWrite(String contentType, long expectEntityLength) {
        ...
        if (expectEntityLength <= 0) {
            Object obj = wrapper.redisTemplate.opsForValue().get(BEFORE_WRITE_PREFIX + ioFile.getId());
            log.info("beforeWrite obj => {} key => {}", obj, BEFORE_WRITE_PREFIX + ioFile.getId());
            synchronized (this) {
                if (Boolean.TRUE.equals(wrapper.redisTemplate.hasKey(BEFORE_WRITE_PREFIX + ioFile.getId()))) {
                    log.info("beforeCreateFile 已判断");
                    return true;
                }
                log.info("-------------------------------->>>>");
                wrapper.redisTemplate.opsForValue().set(BEFORE_WRITE_PREFIX + ioFile.getId(), "", 2, TimeUnit.HOURS);
            }
        }

        log.info("beforeCreateFile 2 >> 前置检查 fileId => {}", this.ioFile.getId());
    ...
    public long write(InputStream content, String contentType, long startIndex, long totalFileLength, long expectEntityLength)
        ...
        log.info("fileImpl write serverPath => {}  startIndex => {} totalFileLength => {} fileId => {} expectEntityLength => {}",
                pathFragment, startIndex, totalFileLength, ioFile.getId(), expectEntityLength);
...
//            log.info("expectEntityLength > 0  => {}", expectEntityLength);
            ...
            commonSource.setSize(expectEntityLength);
            // 转码前需要把下载的文件后缀修改，判断文件是否要改扩展名（场景：钉钉上传）
            if (commonSource.getName().indexOf(".") >= 0) {
                String suffix = commonSource.getName().substring(commonSource.getName().lastIndexOf(".") + 1);
                if ("crdownload".equals(suffix)){
                    String sourceName = commonSource.getName().replace(".crdownload" , "");
                    if (sourceName.indexOf(".") >= 0){
                        String fileType = sourceName.substring(sourceName.lastIndexOf(".") + 1);
                        java.io.File oldFile = new java.io.File(commonSource.getPath());
                        String newPath = commonSource.getPath().replace("."+suffix,"."+fileType);
                        java.io.File newFile = new java.io.File(newPath);
                        boolean isRenamed = oldFile.renameTo(newFile);
                        if (isRenamed){
                            commonSource.setPath(newPath);
                            commonSource.setName(sourceName);
                            commonSource.setFileType(fileType);
                            log.info("expectEntityLength > 0  重命名成功 newPath => {} ioFile => {}", newPath, ioFile);
                        }else {
                            log.info("expectEntityLength > 0  重命名失败 newPath => {} ioFile => {}", newPath, ioFile);
                        }
                    }
                }
            }
            ...
            wrapper.redisTemplate.delete(BEFORE_WRITE_PREFIX + ioFile.getId());
        }
    ...
    public void fileTranscodingAsync() {
        ...
        commonSourceVo.setFileID(ioFile.getId());
