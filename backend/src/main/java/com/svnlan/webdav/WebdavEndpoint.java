...
//            String webdavUrl = generateWebDavUrl(loginUser, ResourceType.PRIVATE);
            String webdavUrl = properties.getHost() + properties.getRootContext().get(0);
            String name = loginUser.getName();
            LogUtil.info("getWindowCmdFile => {}"+ cmdContent + ",webdavUrl=" + webdavUrl);
    ...
    @RequestMapping(path = "${webdav.rootContextPrefix}/**", produces = MediaType.ALL_VALUE, headers = "Connection!=Upgrade")
    public void privateWebDavPreFix(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        log.info("webdav prefix path => {}   method=> {} ", httpServletRequest.getRequestURI(), httpServletRequest.getMethod());

        if (httpServletRequest.getRequestURI().indexOf("/favor") >= 0) {
            log.info("收藏夹仅支持查看2 webdav path => {} method => {}", httpServletRequest.getRequestURI(), httpServletRequest.getMethod());
    ...
    @RequestMapping(path = "${webdav.rootContextPrefix}/**", produces = MediaType.ALL_VALUE, method = {RequestMethod.OPTIONS}, headers = "Connection!=Upgrade")
    public void privateOptionPrefix(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        log.info("options prefix path => {}", httpServletRequest.getRequestURI());
        performDavRequest(httpServletRequest, httpServletResponse);
    }
    @RequestMapping(path = "/**", produces = MediaType.ALL_VALUE, method = {RequestMethod.OPTIONS}, headers = "Connection!=Upgrade")
    public void privateOptionPrefix2(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        log.info("options prefix path => {}", httpServletRequest.getRequestURI());
        performDavRequest(httpServletRequest, httpServletResponse);
    }
    ...
    public void privateWebDav(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        log.info("webdav path => {}  method => {}  ua=> {}", httpServletRequest.getRequestURI(), httpServletRequest.getMethod(), httpServletRequest.getHeader("User-Agent") );
        performDavRequest(httpServletRequest, httpServletResponse);
    }
    ...
    private void performDavRequest(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        ...
        // TODO 用来本地测试，删除(本地与开发环境的redis存的值不同)
