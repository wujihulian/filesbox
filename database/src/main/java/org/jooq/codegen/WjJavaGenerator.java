 ...
 * @author Lukas Eder
 */
public class WjJavaGenerator extends AbstractGenerator {
    ...
    public WjJavaGenerator() {
        this(JAVA);
    }

    WjJavaGenerator(Language language) {
        super(language);
    ...
    private final void generatePojo0(Definition tableOrUDT, JavaWriter out) {
        ...
        if (scala) {
            ...
            for (TypedElementDefinition<?> column : getTypedElements(tableOrUDT)) {
                out.tab(1).println("protected %s%s %s;",
    ...
    protected void generateSchema(SchemaDefinition schema, JavaWriter out) {
        ...
        if (scala) {
            ...
            out.tab(1).javadoc(NO_FURTHER_INSTANCES_ALLOWED);
            out.tab(1).println("private %s() {", className);
            out.tab(2).println("super(\"%s\", null);", "");
            out.tab(1).println("}");
        }
