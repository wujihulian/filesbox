...
Page({
    ...
    tabChange(e){
        let {index} = e.detail,{logType,navList} = this.data
        if(index==0){
            logType = 'user.index.loginSubmit'
            navList = ['时间','系统','客户端','IP地址']
        }else if(index==1){
            ...
            navList = ['时间','位置','状态','文件'],
            logType = 'file.upload,explorer.index.zipDownload,explorer.index.fileOut,explorer.index.fileDownload'
        }
        this.setData({
            navList,
            tabIndex:index,
