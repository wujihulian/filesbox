...
public class IPAddressUtils {
     ...
     * 纯真IP数据库保存的文件夹
     */
    String url = this.getClass().getResource("/qqwry.dat").getPath();
    private String INSTALL_DIR = url;
    ...
    static {
        try {
            ...
            try {
                file = new ClassPathResource("qqwry.dat").getFile();
            ...
            try {
                ...
                // throw new RuntimeException(ex);
                LogUtil.error(e + "静态资源加载ipFile失败 FileNotFoundException");
            }
        } catch (Exception e) {
            e.printStackTrace();
            LogUtil.error(e + "静态资源加载ipFile失败");
        }
    }
    ...
    public void init() {
        try {
            ...
            try {
                if (!file.exists()) {
                    file = new File(this.getClass().getResource("/qqwry.dat").getPath());
                    if (!file.exists()) {
                        file = new File("/qqwry.dat");
                        if (!file.exists()) {
                ...
                if (ipFile == null) {
                    ipFile = new RandomAccessFile(file,
                            "r");
                }
            } catch (FileNotFoundException e) {
                    ...
                    if (files[i].isFile()) {
                        if (files[i].getName().toLowerCase().equals(filename)) {
                            try {
                                ...
                                LOGGER.error("IP地址信息文件没有找到，IP显示功能将无法使用:{}" + e1.getMessage(), e1);
                                ipFile = null;
                            }
                            break;
                        }
            ...
            if (ipFile != null) {
                try {
                    ...
                    LogUtil.error(e, "IP地址信息文件格式有错误，IP显示功能将无法使用" + e.getMessage());
                    ipFile = null;
                }
            }
    ...
    public String getCity(final String ipAddress) {
        try {
            if (ipAddress.startsWith("192.168.")) {
                LOGGER.error("此IP[{}]段不进行处理！", ipAddress);
                return null;
            }
