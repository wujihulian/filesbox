...
public final class PropertiesUtil {
    ...
    /**
       ...
       * @Modified:
       */
    public final static String getUpConfig(String key) {
        Properties pps = getConfig(GlobalConfig.UPCONFIG);
        return pps == null ? null : pps.getProperty(key);

    }

    public final static Properties getConfig(String configName){
        Properties pps = new Properties();
        InputStream upConfig = PropertiesUtil.getCurrentClass().getResourceAsStream("/" + configName);
        try (InputStream in = new BufferedInputStream(upConfig)) {
            pps.load(in);
            return pps;
        } catch (IOException e) {
            e.printStackTrace();
