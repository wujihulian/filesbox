...
public class FileOptionTool {
    ...
    public IOSource addCommonSource(long opUserId, CommonSource commonSource, EventEnum eventEnum,
        ...
        try {
            if (Objects.isNull(ioFile.getFileID())) {
        ...
        if (ObjectUtils.isEmpty(ioFile.getFileID()) || ioFile.getFileID() <= 0) {
            return null;
        }
        ...
        if (!ObjectUtils.isEmpty(commonSource.getIsEdit()) && "1".equals(commonSource.getIsEdit())) {
            ...
            source = this.addIoSourceDetail(commonSource, opUserId, ioFile.getFileID(), eventEnum);
        }
        return source;
    }
    ...
    public void editIoSourceDetail(CommonSource commonSource, Long opUserId) {
        ...
        source.setSourceID(commonSource.getSourceID());
        ...
        // 添加文档操作event
        this.addSourceEvent(source.getSourceID(), commonSource.getParentID(), opUserId, source.getName(), EventEnum.edit);
    }
    ...
    public IOSource addIoSourceDetail(CommonSource commonSource, Long opUserId, Long fileID, EventEnum eventEnum) {
        ...
        if (!ObjectUtils.isEmpty(source.getSourceID()) && source.getSourceID() > 0) {
            commonSource.setSourceID(source.getSourceID());
            ...
            if (Arrays.asList(1, 2).contains(source.getTargetType())) {
                ...
                this.addSourceEvent(source.getSourceID(), commonSource.getParentID(), opUserId, source.getName(), eventEnum);
            }
        }
        return source;
    }
    ...
    public IOSource updateCommonSource(long opUserId, CommonSource commonSource) {
        ...
        ioFile.setPath(commonSource.getPath());
        ioFile.setIsM3u8(0);
        ioFile.setIsH264Preview(commonSource.getIsH264Preview());
        ioFile.setIsPreview(commonSource.getIsPreview());
        ioFile.setAppPreview(commonSource.getAppPreview());
        ioFile.setThumbSize(ObjectUtils.isEmpty(commonSource.getFileThumbSize()) ? 0L : commonSource.getFileThumbSize());
        ioFile.setConvertSize(ObjectUtils.isEmpty(commonSource.getFileConvertSize()) ? 0L : commonSource.getFileConvertSize());
        ...
        IOSource source = new IOSource();
        source.setModifyUser(opUserId);
        source.setFileID(ioFile.getFileID());
        source.setSize(commonSource.getSize());
    ...
    public CommonSource selectByChecksum(String checksum, Long fileSize, Long time) {
    ...
    public boolean moveFile(CheckFileDTO updateFileDTO, LoginUser loginUser) {
        ...
        String desktopSourceIDStr = ioSourceMetaDao.getSourceIDMetaByKey(loginUser.getUserID() + "", "myDesktop");
        ...
        if (fileTargetType == 2){
            ...
            for (IOSource source : moveList) {
                userAuthTool.checkGroupDocAuth(loginUser, source.getSourceID(), source.getParentLevel(), "10", fileTargetType, a == 0);
                a++;
            }
            if (!CollectionUtils.isEmpty(sourceLevelList)) {
                ...
                if (!CollectionUtils.isEmpty(moveListByLevel)){
                    List<Long> otherSourceIds = moveListByLevel.stream().map(IOSource::getSourceID).collect(Collectors.toList());
        ...
        /** 操作日志 */
        Map<Long, IOSource> sourceMap = parentList.stream().collect(Collectors.toMap(IOSource::getSourceID, Function.identity(), (v1, v2) -> v2));
    ...
    public boolean renameFile(CheckFileDTO updateFileDTO, LoginUser loginUser) {
        ...
        for (IOSource ioSource : copyList){
            userAuthTool.checkGroupDocAuth(loginUser, ioSource.getSourceID(), ioSource.getParentLevel(), "8", ioSource.getTargetType());
        }
        Map<Long, String> sourceNameMap = copyList.stream().collect(Collectors.toMap(IOSource::getSourceID, IOSource::getName, (v1, v2) -> v2));
        ...
        Map<Long, IOSource> sourceMap = copyList.stream().collect(Collectors.toMap(IOSource::getSourceID, Function.identity(), (v1, v2) -> v2));
        List<Map<String, Object>> paramList = new ArrayList<>();
        for (SourceOpDto dto : updateFileDTO.getDataArr()) {
        ...
        if (!ObjectUtils.isEmpty(size) && size > 0) {
            if (!ObjectUtils.isEmpty(disk) && !ObjectUtils.isEmpty(size) && size > 0 && !ObjectUtils.isEmpty(disk.getTargetType())) {
                ...
                if (disk.getTargetType().intValue() == 1) {
                    Long userSizeUse = disk.getUserSizeUse() + size;
                    // 0 为不限制容量
                    ...
                    // 部门
                    Long sizeUse = disk.getSizeUse() + size;
                    // 0 为不限制容量
    ...
    public List<IOSource> copyFile(CheckFileDTO updateFileDTO, LoginUser loginUser) {
        ...
        List<IOSource> copyListByLevelCo = null;
        ...
        if (fileTargetType == 2) {
            ...
            for (IOSource source : copyList) {
                // 权限校验
                userAuthTool.checkGroupDocAuth(loginUser, source.getSourceID(), source.getParentLevel(), "4,10", source.getTargetType() , a == 0);
                a ++;
            }
            if (!CollectionUtils.isEmpty(sourceLevelList)) {
                copyListByLevelCo = ioSourceDao.copySourceListByLevel(sourceLevelList);
                List<Long> otherSourceIds = copyListByLevelCo.stream().map(IOSource::getSourceID).collect(Collectors.toList());
        ...
        for (IOSource source : copyList) {
            ...
            source.setTargetType(cs.getTargetType());
            if (!ObjectUtils.isEmpty(sourceName) && sourceName.containsKey(source.getSourceID())) {
                source.setName(sourceName.get(source.getSourceID()));
        ...
        for (IOSource source : copyList) {
            eventList.add(new IoSourceEvent(source.getSourceID(), source.getParentID(), loginUser.getUserID(), EventEnum.copy.getCode(), getSourceEventDesc(EventEnum.copy, source.getName(), "")));
        }
        if (!CollectionUtils.isEmpty(sourceLevelList)) {
    ...
    public void recycleFile(CheckFileDTO updateFileDTO, LoginUser loginUser, boolean isAll) {
        ...
        try {
            ioSourceDao.restoreDirOrFile(sourceIdList, loginUser.getUserID());
        } catch (Exception e) {
        ...
        /** 操作日志 */
        Map<String, Object> reMap = null;
        Map<Long, IOSource> sourceMap = deleteList.stream().collect(Collectors.toMap(IOSource::getSourceID, Function.identity(), (v1, v2) -> v2));
        List<Map<String, Object>> paramList = new ArrayList<>();
    ...
    public void deleteBin(CheckFileDTO updateFileDTO, LoginUser loginUser, boolean isAll) {
        ...
        Map<Long, String> filePathMap = copyList.stream().filter(n -> !ObjectUtils.isEmpty(n.getPath())).collect(Collectors.toMap(CommonSource::getFileID, CommonSource::getPath, (v1, v2) -> v2));
    ...
    public Integer deleteFile(CheckFileDTO updateFileDTO, LoginUser loginUser) {
        ...
        for (IOSource ioSource : deleteList){
            // 权限校验
            userAuthTool.checkGroupDocAuth(loginUser, ioSource.getSourceID(), ioSource.getParentLevel(), "10", ioSource.getTargetType());
        }
        // 判断是否是部门
        ...
        if (!CollectionUtils.isEmpty(paList)) {
            try {
                ioSourceDao.deleteSourceByParent(paList);
            } catch (Exception e) {
    ...
    private void addOperateLog(List<IOSource> ioSourceList, CheckFileDTO updateFileDTO, LoginUser loginUser, LogTypeEnum logTypeEnum) {
        ...
        Map<String, Object> reMap = null;
        Map<Long, IOSource> sourceMap = ioSourceList.stream().collect(Collectors.toMap(IOSource::getSourceID, Function.identity()));
        List<Map<String, Object>> paramList = new ArrayList<>();
        for (SourceOpDto dto : updateFileDTO.getDataArr()) {
            if (!ObjectUtils.isEmpty(sourceMap) && sourceMap.containsKey(dto.getSourceID())) {
                reMap = new HashMap<>(4);
    ...
    public String getAttachmentToken3(String key) {
        ...
        try {
            ...
            LogUtil.error(e, "getAttachmentToken3,验证错误  AES解密失败");
            return "";
        }
        return tokenArr[0];
    }
    ...
    public void makeFile(CheckFileDTO updateFileDTO, LoginUser loginUser, Map<String, Object> resultMap) {
        ...
        // 添加
        this.saveFileDetail(commonSource, updateFileDTO, busType, fileExtension, originalFilename, resultMap, "makeFile");
    ...
    public void saveFileDetail(CommonSource commonSource, CheckFileDTO updateFileDTO, String busType, String fileExtension
            , String originalFilename, Map<String, Object> resultMap, String pre) {
