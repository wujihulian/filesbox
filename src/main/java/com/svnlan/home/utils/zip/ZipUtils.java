...
public class ZipUtils {
    ...
    public static void updateProgress(long nread, long length, StringRedisTemplate stringRedisTemplate, String taskID){
        if (!ObjectUtils.isEmpty(stringRedisTemplate) && nread > 0 && length > 0) {
            ...
            stringRedisTemplate.opsForValue().set(GlobalConfig.progress_key_zip_file +  taskID, String.valueOf(progress), 60, TimeUnit.SECONDS);
    ...
    public static void toZipNewList(String srcDir, String finalFilePath, StringRedisTemplate stringRedisTemplate, CheckFileDTO checkFileDTO
            , boolean isProgress, List<IOSourceVo> attachments, Map<Long, IOSourceVo> mainAttsMap) throws RuntimeException {
        String taskID = checkFileDTO.getTaskID();
        Integer level = checkFileDTO.getLevel();
        try {
            ZipParameters parameters = setZipParameters(level);
            ...
            if (!CollectionUtils.isEmpty(filesToAdd)){
                LogUtil.info("zipProgressMonitor_getPercentDone filesToAdd=" + JsonUtils.beanToJson(filesToAdd) + "， fileNamesMap=" + JsonUtils.beanToJson(fileNamesMap));
                ...
                while (!zipProgressMonitor.getState().equals(ProgressMonitor.State.READY)) {
                    if (isProgress) {
                        double p = zipProgressMonitor.getPercentDone();
                        if (p >= 1){
                            p = p - 1;
                        }
                        stringRedisTemplate.opsForValue().set(GlobalConfig.progress_key_zip_file + taskID, String.valueOf(p), 60, TimeUnit.SECONDS);
                        try {
                            LogUtil.info("zipProgressMonitor_getPercentDone=" + p);
                            Thread.sleep(100);
                        }catch (Exception e){
                            LogUtil.error(e, "zipProgressMonitor_getPercentDone error");
                            break;
                        }
                    }
                }
                if (zipProgressMonitor.getResult().equals(ProgressMonitor.Result.SUCCESS)) {
                    new net.lingala.zip4j.ZipFile(finalFilePath).renameFiles(fileNamesMap);
                    LogUtil.info("zipProgressMonitor_getPercentDone Successfully added folder to zip");
                } else if (zipProgressMonitor.getResult().equals(ProgressMonitor.Result.ERROR)) {
                    LogUtil.info("zipProgressMonitor_getPercentDone Error occurred. Error message: " + zipProgressMonitor.getException().getMessage());
                } else if (zipProgressMonitor.getResult().equals(ProgressMonitor.Result.CANCELLED)) {
                    LogUtil.info("zipProgressMonitor_getPercentDone Task cancelled");
                }
            }
        }catch (Exception e){
            LogUtil.error(e, "");
