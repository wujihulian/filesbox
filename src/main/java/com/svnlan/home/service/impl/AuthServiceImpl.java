...
public class AuthServiceImpl implements AuthService {
    ...
    public List<IoSourceAuthVo> getSourceAuth(CheckFileDTO checkFileDTO, LoginUser loginUser){
        ...
        List<Long> userIdList = list.stream().filter(n-> n.getTargetType().intValue() == 1 && n.getTargetID() > 0).map(IoSourceAuthVo::getTargetID).collect(Collectors.toList());
        Set<Long> groupIDList = list.stream().filter(n-> n.getTargetType().intValue() == 2 && n.getParentID() > 0).map(IoSourceAuthVo::getParentID).collect(Collectors.toSet());
        ...
        for (IoSourceAuthVo authVo : list){
            ...
            if (authVo.getTargetType().intValue() == 1){
                ...
                if (!ObjectUtils.isEmpty(groupMap) && groupMap.containsKey(authVo.getParentID())){
