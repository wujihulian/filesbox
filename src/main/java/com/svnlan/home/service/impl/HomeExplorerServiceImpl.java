...
public class HomeExplorerServiceImpl implements HomeExplorerService {
    ...
    private HomeExplorerResult getUserFavList(HomeExplorerDTO homeExplorerDTO, LoginUser loginUser){
        ...
        List<HomeExplorerVO> list = homeExplorerDao.getUserFavExplorer(hashMap);
        ...
        if (!CollectionUtils.isEmpty(list)) {
            ...
            List<UserVo> userList = isSelect ? userDao.getUserBaseInfo(new ArrayList<>(uidList)) : null;
    ...
    private HomeExplorerResult getRecycleList(HomeExplorerDTO homeExplorerDTO, LoginUser loginUser){
        ...
        hashMap = systemSortTool.setSortAboutMap(systemSortVo,hashMap, loginUser.getUserID(), homeExplorerDTO.getSortField()
                , homeExplorerDTO.getSortType(), key, "io.");



        PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        List<HomeExplorerVO> list = homeExplorerDao.getUserRecycleExplorer(hashMap);
        HomeExplorerResult result = new HomeExplorerResult();
        List<HomeExplorerVO> folderVOList = new ArrayList<>();
        List<HomeExplorerVO> fileVOList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(list)) {
    ...
    private HomeExplorerResult getUserRencentList(HomeExplorerDTO homeExplorerDTO, LoginUser loginUser){
        ...
        PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        List<HomeExplorerVO> list = homeExplorerDao.getUserRencentExplorer(hashMap);
        HomeExplorerResult result = new HomeExplorerResult();
        List<HomeExplorerVO> folderVOList = new ArrayList<>();
        List<HomeExplorerVO> fileVOList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(list)) {
    ...
    private void searchAll(LoginUser loginUser, boolean isSystem, Map<String, Object> hashMap, HomeExplorerVO space){
            ...
            if (isSystem){
                diskList = systemSortTool.getSystemGroupSourceList();
            }else {
    ...
    private HomeExplorerResult getHomeExplorerList(HomeExplorerDTO homeExplorerDTO, LoginUser loginUser){
        ...
        if (!ObjectUtils.isEmpty(homeExplorerDTO.getSourceID()) && homeExplorerDTO.getSourceID() > 0){
            ...
            if (parentInfo.getTargetType().intValue() == 2){
        ...
        if (!ObjectUtils.isEmpty(homeExplorerDTO.getTimeFrom())) {
            ...
            Date timeFrom = DateDUtil.strToDate("yyyy-MM-dd", homeExplorerDTO.getTimeFrom());
            hashMap.put("minDate", timeFrom.getTime()/1000);
        }
        if (!ObjectUtils.isEmpty(homeExplorerDTO.getTimeTo())) {

            Date timeTo = DateDUtil.strToDate("yyyy-MM-dd HH:mm:ss", homeExplorerDTO.getTimeTo() + " 23:59:59");
        ...
        Long total = homeExplorerDao.getCountHomeExplorer(hashMap);
        List<HomeExplorerVO> list = null;
        //PageHelper.startPage(homeExplorerDTO.getCurrentPage(), homeExplorerDTO.getPageSize());
        if(0 < total.longValue()) {
            list = homeExplorerDao.getHomeExplorer(hashMap);
        }
        ...
        if (!CollectionUtils.isEmpty(list)) {
            ...
            for (HomeExplorerVO vo1 : list) {
                ...
                // 是否是部门文件（左侧栏）
                vo1.setIsGroup(0);
                if (isGroupMenu && groupSourceIDs.contains(vo1.getSourceID())) {
                    vo1.setIsGroup(1);
                }
    ...
    public IOSource createDir(HomeExplorerVO homeExplorerVO, LoginUser loginUser ) {
        ...
        if (!ObjectUtils.isEmpty(source.getSourceID()) && source.getSourceID() > 0){
            sourceOperateTool.setSourcePinYin(source.getSourceID(), homeExplorerVO.getName());
            ...
            if (parent.getTargetType().intValue() == 2) {
                ...
                if (!ObjectUtils.isEmpty(groupSource)) {
                    ioSourceMetaDao.insert(new IOSourceMeta(source.getSourceID(), "groupSource", groupSource));
                }
            }
        }

        // 添加文档操作event
        fileOptionTool.addSourceEvent(source.getSourceID(), source.getParentID(), loginUser.getUserID(), source.getName(), EventEnum.mkdir);
    ...
    private HomeExplorerResult getMyMenuList(LoginUser loginUser, List<String> treeOpenList){
        ...
        if (1 == loginUser.getUserType()){
            Integer val = homeExplorerDao.getUserIdentityInfo(loginUser.getUserID());
    ...
    public Map<String, Object> systemOpenInfo(){
        ...
        List<OptionVo> list = systemOptionDao.getSystemConfigByKeyList(Arrays.asList("systemName","systemDesc","passwordRule","needCheckCode", "seo", "seoDesc", "meta", "systemLogo", "browserLogo"
                , "thirdLoginConfig", "registerConfig", "globalIcp","defaultHome"));
        for (OptionVo vo : list){
            if (Arrays.asList("browserLogo","systemLogo").contains(vo.getKey()) && !ObjectUtils.isEmpty(vo.getValue())){
                reMap.put(vo.getKey(), FileUtil.getShowImageUrl(vo.getValue(), vo.getKey()+".png"));
            }else {
                reMap.put(vo.getKey(), ObjectUtils.isEmpty(vo.getValue()) ? "" : vo.getValue());
            }
