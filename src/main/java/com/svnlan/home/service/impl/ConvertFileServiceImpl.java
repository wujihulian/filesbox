        ...
        if (commonSourceVO.getFileID() != null && !commonSourceVO.getFileID().equals(0L)) {
            ...
            if (Arrays.asList(GlobalConfig.yongzhong_doc_type, GlobalConfig.yongzhong_excel_type
                ...
                if (!Arrays.asList(GlobalConfig.IMAGE_TYPE_ARR).contains(sourceSuffix)) {
                    // 转码放外面
                    Date date = new Date();
                    LogUtil.info("doConvert-- begin---" + JsonUtils.beanToJson(commonSourceVO) + "，time=" + DateDUtil.getYearMonthDayHMS(date, DateDUtil.YYYY_MM_DD_HH_MM_SS));
                    convertUtil.doConvert(convertDTO, commonSource);
                    LogUtil.info("doConvert--end--" + JsonUtils.beanToJson(commonSourceVO) + "，time=" + DateDUtil.getYearMonthDayHMS(date, DateDUtil.YYYY_MM_DD_HH_MM_SS));
                }
            }
        }
    }
