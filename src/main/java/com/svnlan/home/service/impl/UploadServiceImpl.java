...
public class UploadServiceImpl implements UploadService {
    ...
    public CommonSourceVO upload(UploadDTO uploadDTO, LoginUser loginUser, CommonSource commonSource) {
        ...
        if (!StringUtil.isEmpty(uploadDTO.getOverrideName())) {
            try {
                ...
                LogUtil.error(e, "upload 设置名称出错, " + uploadDTO.getOverrideName());
            }
        }
        ...
        uploadStateDTO.setChecksum(uploadDTO.getHashMd5());
        //非分段，直接上传的类型
        if (uploadDTO.getChunk() == null || uploadDTO.getChunks() == null || uploadDTO.getChunks().equals(0) || uploadDTO.getChunks().equals(1)) {
            ...
            if (uploadStateDTO.getUpState() && uploadStateDTO.getPartAllDone()) {
                commonSource = this.doMergeFile(uploadStateDTO, commonSource);
                isDone = true;
            }
        }
    ...
    public CommonSourceVO fileUpload(UploadDTO uploadDTO, LoginUser loginUser, CommonSource commonSourceRe) {
        ...
        Long sourceId = 0L;
        //完成合并或直接上传，添加数据
        if (commonSource != null && !StringUtil.isEmpty(commonSource.getHashMd5())) {
            //验证文件大小
            this.doBeforeInsert(uploadDTO, commonSource);
...
//            System.out.println(tempFolderPath);
            ...
            if (!folder.exists()) {
                if (!folder.mkdirs()) {
                    LogUtil.error("创建目录失败 path:" + tempFolderPath);
//                    return uploadStateDTO;
                }
            }
...
//            String chunkMd5 = DigestUtils.md5DigestAsHex(fis);
//            fis.close();
            //todo 单片段md5

            if (!writeSuccess) {
                LogUtil.error("写入文件失败");
                return uploadStateDTO;
            }
//            String md5File = tempFolderPath + checksum + "_md5.txt";
//            FileUtil.putFileContent(md5File,chunkMd5);
            ...
            for (int i = 0; i < chunks; i++) {
                String idxFileName = tempFolderPath + checksum + "_" + i + ".part";
                //某个分段不存在，则表示未完成
            ...
            System.out.println("allDone:" + allDone.toString());
            if (allDone) {
...
//            StringBuilder sb = new StringBuilder();
     ...
     * @Modified:
     */
    private CommonSource doMergeFile(UploadStateDTO uploadStateDTO, CommonSource commonSource) {
        LogUtil.info("merge start. time:" + new Date());
        //业务类型
...
//                return null;
        ...
        if (!Arrays.asList(GlobalConfig.VIDEO_TYPE_ARR).contains(commonSource)){
            ...
            try (FileChannel finalFileChannel = new FileOutputStream(finalFile, true).getChannel();) {
                ...
                //将临时文件合并到最终文件
                LogUtil.info("merge break1. time:" + new Date());
                for (int i = 0; i < chunks; i++) {
                    //拼接临时文件路径
                ...
                LogUtil.info("merge break2. time:" + new Date());
                //设置合并状态为true
                uploadStateDTO.setMergeState(true);
            } catch (Exception e) {
                LogUtil.error(e, "合并文件失败" + JsonUtils.beanToJson(commonSource));
                return null;
            }
        ...
        try {
            ...
            uploadStateDTO.setMergeState(true);
            //最终合并文件md5
            LogUtil.info("merge break2.5time:" + new Date());
            ...
            commonSource.setFileType(fileExtension);
            commonSource.setSourceType(typeCode);
            LogUtil.info("file info : " + JsonUtils.beanToJson(commonSource));

        } catch (Exception e) {
            LogUtil.error(e, "合并文件失败" + JsonUtils.beanToJson(commonSource));
            return null;
        }
        LogUtil.info("merge finished. time:" + new Date());
        return commonSource;
    }
    ...
    public Map<String, Object> checkFile(CheckFileDTO checkFileDTO, LoginUser loginUser) {
        ...
        List<String> sourceNameList = ioSourceDao.getSourceNameList(checkFileDTO.getSourceID());
        ...
        CommonSource commonSource = fileOptionTool.selectByChecksum(checkFileDTO.getHashMd5(), checkFileDTO.getSize(), time);
...
*/
    ...
    public CommonSource fileSave(SaveUploadDTO uploadDTO, LoginUser loginUser) {
        ...
        if (finalFile.exists()) {
            ...
            sourceUpdate.setSourceID(uploadDTO.getSourceID());
    ...
    private void saveNewFile(SaveUploadDTO uploadDTO, LoginUser loginUser, CommonSource source, CommonSource commonSource2) {
        ...
        fileOptionTool.saveFileDetail(commonSource2, updateFileDTO, busType, source.getFileType(), source.getName(), resultMap, "fileSave");
