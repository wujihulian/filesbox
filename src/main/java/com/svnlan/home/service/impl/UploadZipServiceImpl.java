...
public class UploadZipServiceImpl implements UploadZipService {
    ...
    public boolean zipFile(CheckFileDTO checkFileDTO, Map<String, Object> resultMap, LoginUser loginUser){
        ...
        // 存放复制文件的临时文件名
        String tempFolder = uuid + System.currentTimeMillis() + "_" + userID ;
        String finalFilePath = null;
        if (isDown){
            // 压缩包的路径及名称
            finalFilePath = finalFolderPath + fileName;
        }else {
            // 压缩包的路径及名称
...
//                return null;
        ...
        if (ObjectUtils.isEmpty(checkFileDTO.getFullName())){
            ...
            loginUser.setServerName(HttpUtil.getServerName(request));
            ...
            , systemLogTool,fileOptionTool, busTypeHandleService, uploadDTO, parentSource, sourceNameList, targetType, serverUrl);
