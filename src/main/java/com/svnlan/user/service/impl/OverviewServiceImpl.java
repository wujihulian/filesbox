...
public class OverviewServiceImpl implements OverviewService {
    ...
    public Object getFileRankingAndProportion(Integer topCount) {
        // 获取文件占比数据
        CompletableFuture<List<JSONObject>> fileProportionFuture = CompletableFuture.supplyAsync(this::queryFileProportion, asyncTaskExecutor);
        CompletableFuture<List<JSONObject>> UserSpaceFuture = CompletableFuture.supplyAsync(() -> queryUserSpaceFileUsageRankList(topCount), asyncTaskExecutor);
        CompletableFuture<List<JSONObject>> groupSpaceFuture = CompletableFuture.supplyAsync(this::getGroupSpaceFileUsageList, asyncTaskExecutor);
        CompletableFuture<List<JSONObject>> targetTypeProportionFuture = CompletableFuture.supplyAsync(this::getTargetTypeProportion, asyncTaskExecutor);
     ...
     * 查询文件类型占比
     */
    private List<JSONObject> queryFileProportion() {
        ...
        if (CollectionUtils.isEmpty(jsonObjList)) {
            jsonObjList = ioSourceDao.selectFileProportion();
     ...
     * 获取用户个人中心文件占用排名
     */
    private List<JSONObject> queryUserSpaceFileUsageRankList(Integer topCount) {
        List<JSONObject> jsonObjList = CaffeineUtil.USER_SPACE_RANK_CACHE.getIfPresent(USER_SPACE_RANK);
        if (CollectionUtils.isEmpty(jsonObjList)) {
            jsonObjList = userDao.getUserSpaceRank(topCount);
