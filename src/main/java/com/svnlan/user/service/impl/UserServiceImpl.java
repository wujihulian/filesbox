...
public class UserServiceImpl implements UserService, ApplicationContextAware {
        ...
        switch (userOption.getKey()) {
            ...
            case "password":
                ...
                if (StringUtils.hasText(userOption.getSig())) {
                    ...
                    // 普通修改密码方式
                    if (ObjectUtils.isEmpty(userOption.getValue()) || ObjectUtils.isEmpty(userOption.getPassword())) {
                        throw new SvnlanRuntimeException(CodeMessageEnum.shareErrorParam.getCode());
                    }
                    // 解析出原密码
            ...
            case "desktopBackgroundImg":
                ...
                if (ObjectUtils.isEmpty(backImgVo)) {
                    ...
                    paramList.add(new UserOption(loginUser.getUserID(), "", "desktopBackgroundImg", userOption.getValue()));
                    try {
                        userOptionDao.batchInsert(paramList);
                    } catch (Exception e) {
                        LogUtil.error(e, " setUserInfo desktopBackgroundImg insert error userOption=" + JsonUtils.beanToJson(userOption) + "，loginUser=" + JsonUtils.beanToJson(loginUser));
                        throw new SvnlanRuntimeException(CodeMessageEnum.saveError.getCode());
                    }
                } else {
                    try {
                        ...
                        LogUtil.error(e, prefix + " setUserInfo desktopBackgroundImg update error userOption=" + JsonUtils.beanToJson(userOption) + "，loginUser=" + JsonUtils.beanToJson(loginUser));
                        throw new SvnlanRuntimeException(CodeMessageEnum.saveError.getCode());
                    }
                }
                break;
    ...
    public Boolean checkEmailCode(UserDTO optionDTO) {
        ...
        if (!checkCode(FindTypeEnum.EMAIL.getCode() + "_" + SendOperateTypeEnum.BINDING.getCode(), optionDTO.getCode(), optionDTO.getValue(), false)) {
            throw new SvnlanRuntimeException(CodeMessageEnum.codeError.getCode());
        }
        return true;
    }
    ...
    private Boolean sendVerificationCode(String type, String detail, LoginUser loginUser, String operateType, String module, HttpServletRequest request) {
    ...
    private Boolean sendEmailCode(String detail, String code) {
        try {
            String emailTitle = "【" + systemOptionDao.getSystemConfigByKey("systemName") + "】";
...
//        logWithBind(loginUser, LogTypeEnum.wechatUnbind);
    ...
    public LoginQRVO scanLogin(String prefix, LoginParamDTO paramDTO, HttpServletRequest request, HttpServletResponse response) {
        ...
        //
        UserVo user = this.userDao.getUserInfo(targetUserId);
    ...
    public String loginByService(UserVo user, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (tokenMap != null && tokenMap.containsKey("token")) {
            ...
            LogUtil.error("登录失败 else, " + JsonUtils.beanToJson(userDTO));
