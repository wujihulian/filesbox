...
public class EnWechatServiceImpl extends ThirdBaseService implements EnWechatService {
    ...
    public static final String EN_WECHAT_ACCESS_TOKEN_KEY = "accessToken:enWechat:%s";
    ...
    public String getAccessToken() {
        String accessToken = stringRedisTemplate.opsForValue().get(String.format(EN_WECHAT_ACCESS_TOKEN_KEY, corpId));
        if (!StringUtils.hasText(accessToken)) {
            synchronized (this) {
                accessToken = stringRedisTemplate.opsForValue().get(String.format(EN_WECHAT_ACCESS_TOKEN_KEY, corpId));
                if (!StringUtils.hasText(accessToken)) {
                    ...
                    if (Objects.equals(jsonObj.getInteger("errcode"), 0)
                        ...
                        stringRedisTemplate.opsForValue().set(String.format(EN_WECHAT_ACCESS_TOKEN_KEY, corpId), accessToken, 7100, TimeUnit.SECONDS);
                    } else {
    ...
    public Pair<String, String> beforeLoginWithWechat(String type) {
        return beforeLogin(type, () -> corpId);
    }
