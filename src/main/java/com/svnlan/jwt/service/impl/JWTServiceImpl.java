...
public class JWTServiceImpl implements JWTService {
    ...
    public Map<String, String> doLogin(UserDTO userDTO, HttpServletRequest request) {
        ...
        String passwordErrorLock = systemOptionDao.getSystemConfigByKey("passwordErrorLock");
        ...
        if (user == null) {
            user = userJwtDao.findByUserName(username);
        }
        //账号是否正确
        ...
        if (user.size() == 1) {
            ...
            if (ObjectUtils.isEmpty(loginUser)) {
                LogUtil.error("登录结果有多个" + JsonUtils.beanToJson(userDTO));
...
//        String tokenKey = "token";
    ...
    public Map<String, String> register(UserDTO userDTO, HttpServletRequest request) {
        ...
        String registerConfig = systemOptionDao.getSystemConfigByKey("registerConfig");
