...
public abstract class ThirdBaseService {
    ...
    @SneakyThrows
    public Pair<String, String> beforeLogin(String type, Supplier<String> supplier) {
        // 返回给前端 appId
...
//        String sig = PBE.encrypt(PasswordUtil.MD5(appId, GlobalConfig.PWD_SALT) + "_" + new Date().getTime());
        ...
        if (!CollectionUtils.isEmpty(userIds)) {
            if (userIds.size() == 1) {
                if (Objects.equals(userIds.get(0), userId)) {
                    throw new SvnlanRuntimeException("已绑定该账号");
                }
            }
            throw new SvnlanRuntimeException("已绑定其他账号，请先进行解绑");
        }
    }
    ...
    protected Map<String, String> executeUser(String accessToken, SecurityTypeEnum typeEnum, String state, String code) {
        ...
        log.info("thirdUser => {}", thirdUser);
        String openId = thirdUser.openId;
        List<UserVo> userVoList = userDao.queryUserInfoByOpenIdOrMobile(openId, thirdUser.mobile, typeEnum.getCode());
        UserVo userVo = null;
        ...
        if (Objects.nonNull(userVo)) {
            ...
            if ("changePwd".equals(state)) {
                ...
                // 生成加密字符串
                String value = String.format("%d_%d_changePwd", userVo.getUserID(), new Date().getTime());
            ...
            if (!CollectionUtils.isEmpty(groupIds)) {
                // 查询 group
                List<Group> groupInfoList = groupDao.getGroupInfoList(groupIds);
    ...
    protected JSONObject afterExecuteUser(Long userId) {
        // 根据 userId 查询昵称和头像
        User user = userDao.selectOne(new QueryWrapper<User>().eq("userId", userId).select("avatar", "nickname"));
