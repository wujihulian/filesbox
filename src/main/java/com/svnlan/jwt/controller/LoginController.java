...
//    @LockAction
    @ApiOperation("登录")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "body", name = "userDTO", value = "{" +
                    "  \"encrypted\": false," +
                    "  \"forNotTeacher\": true," +
                    "  \"forTeacher\": true," +
                    "  \"fromPlatform\": true," +
                    ...
                    "  \"test\": \"string\"," +
                    "  \"username\": \"jiaxiaozi\"" +
                    "}", defaultValue = "1", access = "2", example = "3", format = "4", collectionFormat = "5"),
    ...
    public Result login(@RequestBody @Valid UserDTO userDTO, HttpServletRequest request) {
        try {
            ...
            LogUtil.error(e, "登录失败, svn" + JsonUtils.beanToJson(userDTO));
            return new Result(false, e.getErrorCode(), null);
        } catch (Exception e) {
            LogUtil.error(e, "登录失败,e " + JsonUtils.beanToJson(userDTO));
        }
        return new Result(false, CodeMessageEnum.pwdError.getCode(), null);
    }
    ...
    public Result login(@RequestBody LoginUser loginUser) {
        try {
            ...
            LogUtil.error(e, "登录失败" + JsonUtils.beanToJson(loginUser));
        }
...
//        return null;
    ...
    public Result loginWithDingTalk(@RequestParam(value = "authCode") String authCode, @RequestParam(value = "state") String state) throws Exception {
        ...
        if ("innerLogin".equals(state)) {
            ...
            log.info("innerLogin accessToken => {}", accessToken);
            return Result.returnSuccess(dingService.checkAndExecuteInnerUserLogin(accessToken, authCode));
        } else {
            // 钉钉三方登录获取token
            accessToken = dingService.getAccessToken(authCode);
            log.info("common accessToken => {}", accessToken);
    ...
    public Result loginWithEnWechat(@RequestParam(value = "code") String code, @RequestParam(value = "state") String state) {
        ...
        String accessToken = enWechatService.getAccessToken();
    ...
    public Result beforeLoginWithWechat(String type) {
        ...
        Assert.notNull(pair, "暂不支持的 type type=" + type);
        return Result.returnSuccess(new JSONObject().fluentPut("appId", pair.getFirst()).fluentPut("sig", pair.getSecond()));
    }
    ...
    public Result register(@RequestBody @Valid UserDTO userDTO, HttpServletRequest request) {
        try {
            ...
            if (tokenMap != null && tokenMap.containsKey("token")) {
                ...
                LogUtil.error("注册失败 else, " + JsonUtils.beanToJson(userDTO));
                return new Result(false, CodeMessageEnum.pwdError.getCode(), null);
            }
        } catch (SvnlanRuntimeException e) {
            LogUtil.error(e, "注册失败, svn" + JsonUtils.beanToJson(userDTO));
            return new Result(false, e.getErrorCode(), null);
        } catch (Exception e) {
            LogUtil.error(e, "注册失败,e " + JsonUtils.beanToJson(userDTO));
        }
        return new Result(false, CodeMessageEnum.pwdError.getCode(), null);
    }
}
