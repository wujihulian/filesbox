...
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image rightImage:(UIImage *)rightImage handler:(void (^)(YCMenuAction *action))handler{
    YCMenuAction *action = [[YCMenuAction alloc] initWithTitle:title image:image rightImage:rightImage   handler:handler];
    return action;
}
...
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image rightImage:(UIImage *)rightImage handler:(void (^)(YCMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
- (CGPoint)getRefPoint{
    ...
    CGPoint refPoint;
    CGFloat     height = (_actions.count > _maxDisplayCount) ? _maxDisplayCount * _menuCellHeight: _actions.count * _menuCellHeight ;

    if (absoluteRect.origin.y + absoluteRect.size.height + height > kScreenHeight - 10) {
        if(absoluteRect.origin.y<height){
            _isReverse = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_separatorIndexArray containsObject:@(indexPath.row) ]) {
        cell.separatorColor = _separatorColor;//_separatorColor;
        cell.isShowSeparator = YES;
    }else{
        cell.separatorColor = UIColor.clearColor;//_separatorColor;
    }
    cell.accessoryType = UITableViewCellAccessoryNone;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (action.rightImage) {
        if ([action.rightImage isEqual:VULGetImage(@"icon_right")]) {
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }else{
            cell.rightImageV.image =action.rightImage;
    ...
    [cell.textLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.mas_equalTo(cell.imageView .mas_centerY);
    }];

    cell.imageView.image = action.image ? action.image:getImageWithColor(HEXCOLOR(0xffffff));
    if (self.tintColor) {
    ...
    if (isPermissionWithModel(action.title,@[[VULFileObjectModel new]]) || self.isNoPre) {
        cell.textLabel.textColor = _textColor;
