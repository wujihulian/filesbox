...
@end
@implementation VULTMeImageViewCell
...
- (void)clickImage{
    if ([_model.msg hasSuffix:@"svg"]) {
        return;
    }
    XLPhotoBrowser *browser = [XLPhotoBrowser showPhotoBrowserWithImages:@[[NSURL URLWithString:resultsUrl(_model.msg)]] currentImageIndex:0];
    ...
    if ([model.msg hasSuffix:@"gif"]) {
        ...
        [self.imageV sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", ChooseUrl, model.msg]] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (image) {
                ...
                [self.imageGifV mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.top.mas_equalTo(self.nameLabel.mas_bottom).offset(5);
                    make.right.mas_equalTo(-fontAuto(63));
...
//    NSString *imageUrl = [NSString stringWithFormat:@"%@%@", ChooseUrl, model.msg];
//    NSDictionary *dic = [[NSUserDefaults standardUserDefaults] objectForKey:imageUrl];
//    NSNumber *dicWith;
//    NSNumber *dicHeight;
//    CGSize imageSize;
//    CGFloat imageW = fontAuto(80);
//    CGFloat imageWidth;
//    CGFloat imageHight;
//    if (dic) {
//        dicWith = dic[@"width"];
//        dicHeight = dic[@"height"];
//        imageSize.width =dicWith.floatValue ;
//        imageSize.height =dicHeight.floatValue;
//    }
//
//    [self.imageV mas_remakeConstraints:^(MASConstraintMaker *make) {
//        make.top.mas_equalTo(self.nameLabel.mas_bottom).offset(5);
...
//    [self layoutIfNeeded];
//
//    UIBezierPath *maskOnePath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, imageWidth, imageHight) byRoundingCorners:UIRectCornerTopLeft | UIRectCornerBottomLeft |  UIRectCornerBottomRight cornerRadii:CGSizeMake(10, 10)];
//
