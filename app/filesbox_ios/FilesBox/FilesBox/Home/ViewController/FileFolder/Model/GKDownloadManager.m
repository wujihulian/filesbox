...
- (NSMutableArray <GKDownloadModel *> *)downloadFileList {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        return [GKDownloadModel mj_objectArrayWithFile:[self downloadDataFilePath]];
    }
    return nil;
}
...
- (void)writeToFileWithModelArr:(NSArray <GKDownloadModel *> *)modelArr {
    NSArray *dataArr = [GKDownloadModel mj_keyValuesArrayWithObjectArray:modelArr];
    
    [dataArr writeToFile:[self downloadDataFilePath] atomically:YES];
}

- (BOOL)ifExistDownloadModelWithID:(NSString *)ID {
    __block BOOL exist = NO;
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSArray *modelArr = [GKDownloadModel mj_objectArrayWithFile:[self downloadDataFilePath]];
