...
@property (weak  , nonatomic) UILabel                     *showMonthView;
...
//    label.textColor = HEXCOLOR(0xE3EDFF);
//    label.font = IS_IPAD ? [UIFont systemFontOfSize:320] : [UIFont systemFontOfSize:220];
////    label.font = [UIFont fontWithName:@"PingFangSC-Heavy" size:220];
...
//    NSArray *array = [currentDateString componentsSeparatedByString:@"-"];
//    NSString *currentMonth = array[1];
//    label.text = [NSString stringWithFormat:@"%ld",[currentMonth integerValue]];
    
//    [contentView addSubview:label];
//    [contentView sendSubviewToBack:label];
//    self.showMonthView = label;
...
//                    _showMonthView.frame = _daysContainer.frame;
                    break;
                }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isSelectedToday && ![self.today isEqual:selectedDate]) {
        FSCalendarCell *todayCell = [self cellForDate:self.today atMonthPosition:FSCalendarMonthPositionCurrent];
        NSIndexPath *indexPath = [collectionView indexPathForCell:todayCell];
        if (indexPath) {
            [collectionView deselectItemAtIndexPath:indexPath animated:NO];
        }
        todayCell.selected = NO;
        [todayCell configureAppearance];
        [self deselectCounterpartDate:self.today];
    }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if (shouldTriggerPageChange) {
        ...
        [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        //NSDate转NSString
        NSString *currentDateString = [dateFormatter stringFromDate:_currentPage];
        
        NSArray *array = [currentDateString componentsSeparatedByString:@"-"];
        NSString *currentMonth = array[1];
//        self.showMonthView.text = [NSString stringWithFormat:@"%ld",[currentMonth integerValue]];
    }
...
- (void)setCurrentPage:(NSDate *)currentPage animated:(BOOL)animated
{
    ...
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    //NSDate转NSString
    NSString *currentDateString = [dateFormatter stringFromDate:currentPage];
...
- (void)signinWithSelectDate:(NSDate *)date isDeletion:(BOOL)isDeletion {
    NSDate *targetDate = [self.gregorian dateBySettingHour:0 minute:0 second:0 ofDate:date options:0];
    NSIndexPath *targetIndexPath = [self.calculator indexPathForDate:targetDate];
    
    [_collectionView selectItemAtIndexPath:targetIndexPath animated:NO scrollPosition:UICollectionViewScrollPositionNone];
    FSCalendarCell *cell = (FSCalendarCell *)[_collectionView cellForItemAtIndexPath:targetIndexPath];
    [cell configCellSelecting:isDeletion];
}
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.dateIsToday && self.isSelectedToday) {
        cell.selected = YES;
    }
