...
- (void)layoutSubviews
{
    ...
    _imageView.frame = CGRectMake(self.preferredImageOffset.x, self.preferredImageOffset.y - 7, self.contentView.fs_width, self.contentView.fs_height);
    ...
    CGFloat scaleValue = _appearance.isAttendanceStyle ? 0.1 : 0.5;
    CGPathRef path = [UIBezierPath bezierPathWithRoundedRect:_shapeLayer.bounds
                                                cornerRadius:CGRectGetWidth(_shapeLayer.bounds)*scaleValue*self.borderRadius].CGPath;
...
- (void)configCellSelecting:(BOOL)isDeletion {
    _shapeLayer.opacity = 1;
    ...
    if (![textColor isEqual:_titleLabel.textColor]) {
        _titleLabel.textColor = isDeletion ? HEXCOLOR(0x333333) : textColor;
    }
    ...
    //FF3B30 07C161
    UIColor *borderColor = HEXCOLOR(0xFF3B30);
    UIColor *fillColor = HEXCOLOR(0x07C161);
...
- (void)configureAppearance
{
    ...
    if (!shouldHideShapeLayer) {
        ...
        if (!CGColorEqualToColor(_shapeLayer.strokeColor, cellBorderColor) && self.dateIsToday && !_appearance.isTeacherSignin) {
            _shapeLayer.strokeColor = cellBorderColor;
        } else {
