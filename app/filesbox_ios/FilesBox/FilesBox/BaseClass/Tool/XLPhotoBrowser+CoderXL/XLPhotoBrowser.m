...
//    [saveButton setTitle:KLanguage(@"编辑") forState:UIControlStateNormal];
//    [saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [saveButton setImage:VULGetImage(@"icon_image_edit") forState:UIControlStateNormal];
...
//    [saveButton1 setTitle:@"提取文字" forState:UIControlStateNormal];
...
//    saveButton1.clipsToBounds = YES;
    [saveButton1 setImage:VULGetImage(@"icon_image_text") forState:UIControlStateNormal];

    [saveButton1 addTarget:self action:@selector(extractText) forControlEvents:UIControlEventTouchUpInside];
    self.saveButton = saveButton;
    self.extractBtn = saveButton1;
...
- (void)updateFrames
{
    ...
    self.saveButton.frame = CGRectMake(self.xl_width-60, self.xl_height - 60-K_BottomBar_Height, 40, 40);
    self.extractBtn.frame = CGRectMake(self.xl_width-115, self.xl_height - 60-K_BottomBar_Height, 40, 40);

    self.indexLabel.bounds = CGRectMake(0, 0, 80, 30);
    self.indexLabel.xl_centerX = self.xl_width * 0.5;
    self.indexLabel.xl_centerY = K_StatusBar_Height+15;
...
- (void)saveImage
{
    ...
    if(self.eidtWithImage){
        self.eidtWithImage(zoomingScrollView.currentImage,self.currentImageIndex);
    }
    return;
...
-(void)extractText{
    ...
    if (@available(iOS 11.0, *)) {
        VNImageRequestHandler *handler = [[VNImageRequestHandler alloc] initWithCGImage:zoomingScrollView.currentImage.CGImage options:@{}];
        ...
        if (@available(iOS 13.0, *)) {
            VNRecognizeTextRequest *request = [[VNRecognizeTextRequest alloc] initWithCompletionHandler:^(VNRequest * _Nonnull request, NSError * _Nullable error) {
                ...
                for (VNRecognizedTextObservation *observation in observations) {
                    if (@available(iOS 13.0, *)) {
                        NSArray *arr = [observation topCandidates:1];
                        for (VNRecognizedText *text in arr) {
                ...
                if(recognizedText.length>0){
                    [VULQueue executeInMainQueue:^{
                        FBTextShowView *view= [[FBTextShowView alloc] initWithFrame:CGRectMake(0, 0, VULSCREEN_WIDTH, VULSCREEN_HEIGHT*0.5)];
                        view.textView.text = recognizedText;
                        zhPopupController  *popup =[[zhPopupController alloc] initWithView:view size:CGSizeMake(VULSCREEN_WIDTH, view.height)];
                        popup.layoutType = zhPopupLayoutTypeBottom;
                        popup.presentationStyle = zhPopupSlideStyleFromBottom;
                        [popup showInView:self duration:0.35 delay:0 options:UIViewAnimationOptionCurveEaseInOut bounced:NO completion:nil];

                    }];
                }else{
                    [kWindow makeToast:KLanguage(@"未扫描到文字")];
                }
...
- (void)dismiss
{
    ...
    [UIView animateWithDuration:XLPhotoBrowserHideImageAnimationDuration animations:^{
        ...
        [VULNotificationCenter postNotificationName:@"shareWaterViewNotificationCenter" object:nil];
