...
@interface VULGIFImageView ()

@property (nonatomic, strong) VULGIFImage *animatedImage;
...
@implementation VULGIFImageView
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image isKindOfClass:[VULGIFImage class]] && image.images) {
        if ([image.images[0] isKindOfClass:UIImage.class]) [super setImage:image.images[0]];
        else [super setImage:nil];
        self.currentFrame = nil;
        self.animatedImage = (VULGIFImage *)image;
...
- (void)setAnimatedImage:(VULGIFImage *)animatedImage
{
...
- (void)displayLayer:(CALayer *)layer
{
    ...
    if (self.currentFrame && ![self.currentFrame isKindOfClass:[NSNull class]]) layer.contents = (__bridge id)([self.currentFrame CGImage]);
}
